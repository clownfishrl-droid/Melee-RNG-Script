...
local function meleeFireButtonSignals(btn)
    if not btn then return false end
    local okAny = false
    pcall(function() btn:Activate() okAny = true end)
    if fireGuiButtonClick and fireGuiButtonClick(btn) then okAny = true end

    local fs = rawget(_G, "firesignal")
    if type(fs) ~= "function" and type(firesignal) == "function" then fs = firesignal end
    if type(fs) == "function" then
        pcall(function() fs(btn.Activated) okAny = true end)
        pcall(function() fs(btn.MouseButton1Click) okAny = true end)
        pcall(function() fs(btn.MouseButton1Down) okAny = true end)
        pcall(function() fs(btn.MouseButton1Up) okAny = true end)
    end
    return okAny
end
...
local function meleeVirtualClickGuiObject(obj, repeats)
    ...
    if not okSvc or not vim then return false end

    local ok = false
    repeats = math.clamp(math.floor(tonumber(repeats) or 2), 1, 5)
    for _ = 1, repeats do
...
local function meleeRobustClickGuiTarget(obj, reason)
    ...
    -- Prefer actual GuiButton descendants when they exist.
    local btn = meleeFindClickableDescendant(obj)
    local signaled = false
    if btn and meleeGuiIsActuallyVisible(btn) then
        signaled = meleeFireButtonSignals(btn)
    end

    -- Important fallback for this game: StartBtn can be a Frame with InputBegan.
    -- A center mouse click is closer to what the player actually does.
    local clickedCenter = meleeVirtualClickGuiObject(obj, 2)
    if not clickedCenter and btn and btn ~= obj then
        clickedCenter = meleeVirtualClickGuiObject(btn, 2)
    end

    if signaled or clickedCenter then
        vprint("[MeleeRNG/TimeTrial] clicked", tostring(obj:GetFullName()), tostring(reason or ""))
        return true
    end
    return false
end
...
local function meleeFindTimeTrialStartRunButton()
    ...
    -- Exact path from your Explorer screenshot: PlayerGui.TimeTrialsUI.StartBtn
    ...
    -- Alternate live path seen in MainGUI.GeneralUI.
    local main = pg:FindFirstChild("MainGUI")
    local general = main and main:FindFirstChild("GeneralUI")
    local lobbyFrame = general and general:FindFirstChild("TimeTrialsLobbyFrame", true)
    if lobbyFrame then
...
local function meleeFindTimeTrialLeaveButton()
    ...
    -- Exact path from your Explorer screenshot: PlayerGui.MainGUI.GeneralUI.LeaveTimeTrialBtn
    local main = pg:FindFirstChild("MainGUI")
    local general = main and main:FindFirstChild("GeneralUI")
    local direct = general and general:FindFirstChild("LeaveTimeTrialBtn", true)
    if direct and direct:IsA("GuiObject") and meleeGuiIsActuallyVisible(direct) then
        return direct
    end
...
function meleeTimeTrialClickStartRunBurst(seconds, interval, reason)
    _ttStartRunBurstUntil = math.max(_ttStartRunBurstUntil, os.clock() + (tonumber(seconds) or 7))
    ...
    task.spawn(function()
        while GEN == _G.MeleeRNG_Gen and os.clock() < _ttStartRunBurstUntil do
            if meleeTimeTrialClickStartRun(reason or "burst") then
                ...
                task.wait(tonumber(interval) or 0.20)
            end
        end
...
function meleeMountSafeDungeonMobTimeTrialAddons(MiscTab, SLbl, C)
    ...
    MiscTab:Toggle("Auto Start Time Trial", "Fires StartTimeTrial on a slow timer while ON.", states.timeTrialAutoStart, function(on)
        ...
        if states.timeTrialAutoStart and states.timeTrialAutoClickStartRun then
            meleeTimeTrialClickStartRunBurst(8, 0.18, "auto-start toggle")
        end
    end)
    MiscTab:Toggle("Auto click Start Run popup", "When the Time Trials Lobby appears, clicks the green Start Run button automatically.", states.timeTrialAutoClickStartRun, function(on)
        ...
        if states.timeTrialAutoClickStartRun then
            meleeTimeTrialClickStartRunBurst(6, 0.18, "toggle")
        end
    end)
    ...
    MiscTab:Button("🖱️", "Click Start Run now", "Looks for the Time Trials Lobby Start Run button and clicks it.", C.green, function()
        ...
        if not ok then meleeTimeTrialClickStartRunBurst(4, 0.18, "manual wait") end
    ...
    MiscTab:Button("▶️", "Start Time Trial once", "Calls the existing StartTimeTrial remote once, then auto-clicks Start Run if the lobby opens.", C.green, function()
        ...
        if states.timeTrialAutoClickStartRun then
            meleeTimeTrialClickStartRunBurst(12, 0.20, "start once")
        end
    ...
    task.defer(function()
        ...
        if lobby and (lobby:IsA("RemoteEvent") or lobby.ClassName == "UnreliableRemoteEvent") then
            addConn(lobby.OnClientEvent:Connect(function()
                if states.timeTrialAutoClickStartRun then
                    meleeTimeTrialClickStartRunBurst(7, 0.18, "lobby event")
                end
            end))
        end
    end)
    addThread(task.spawn(function()
        while true do
            ...
            if states.timeTrialAutoStart then
                ...
                if states.timeTrialAutoClickStartRun then
                    meleeTimeTrialClickStartRunBurst(12, 0.20, "auto loop")
                end
                task.wait(6)
            else
                task.wait(1)
                ...
                if room and room ~= lastLeaveModel and os.clock() >= nextLeaveAllowed and meleeFindTimeTrialLeaveButton() then
                    ...
                    if meleeTimeTrialClickLeave("auto room target") then
                        task.delay(1.25, function()
                            ...
                            pcall(function() fireR("StartTimeTrial") end)
                            if states.timeTrialAutoClickStartRun then
                                meleeTimeTrialClickStartRunBurst(12, 0.20, "auto restart after leave")
                            end
                        end)
                    end
                end
