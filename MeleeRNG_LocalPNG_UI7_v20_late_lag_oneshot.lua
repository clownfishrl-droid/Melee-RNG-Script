...
local states = {
    ...
    autoReExec     = false,
    hitboxPulse    = false,
    hitboxDutyCycle  = 4,  -- BIG on 1 frame per N heartbeats (4 = stronger one-shot without old constant BIG lag)
    ...
    -- Added stability controls: kept out of the big UI block as much as possible.
    autoEquipBestInterval = 45, -- seconds between EquipBest calls; higher = fewer weapon rebuild spikes
    dungeonAutoTpNew = false, -- workspace.GeneratedDungeon.Room_N -> BASE + 1 stud
    dungeonRoomInput = "1", -- typed Room_N target, saved as text
    dungeonAutoTpEntered = false, -- keep trying to TP to entered Room_N when it exists
    timeTrialAutoStart = false, -- periodically StartTimeTrial using existing remote
    perfMobsSpawnOnMe = false, -- move workspace.Mobs models to HRP when they appear
    perfHardHideMobs = false, -- stash/disable mob visuals while keeping roots alive
}
...
states.autoEquipBestInterval = math.clamp(math.floor(tonumber(states.autoEquipBestInterval) or 45), 10, 300)
if type(states.dungeonRoomInput) ~= "string" then states.dungeonRoomInput = tostring(states.dungeonRoomInput or "1") end
...
states.perfMobsSpawnOnMe = states.perfMobsSpawnOnMe == true
...
-- ── Extra stable additions: GeneratedDungeon, mob anti-lag, Time Trials ─────
-- Kept outside the UI IIFE so adding sections does not push the main UI over Luau local limits.
local _mrngDungeonLastTpRoom = nil
local _mrngDungeonAutoStarted = false
local _mrngDungeonLatestCached = nil
local _mrngDungeonLatestN = -1
local _mrngDungeonLastEnteredTp = nil
local _mrngDungeonLastEnteredAt = 0
local _mrngMobPerfStarted = false
local _mrngTTStarted = false
local _mrngTTRequired = nil
local _mrngTTCurrent = nil
local _mrngTTTimeLeft = nil
local _mrngMobPartParent = {}
local _mrngMobFxEnabled = {}
local _mrngMobLastMove = {}
local MRNG_HIDDEN_MOBS_GUI = "MeleeRNG_HiddenMobVisuals"
-- Lag guard: never rescan/reparent every mob every frame.  New mobs are handled
-- once on ChildAdded, then a slow capped cleanup pass catches missed mobs.
local MRNG_MOB_SWEEP_SEC = 2.5
local MRNG_MOB_SWEEP_LIMIT = 20
local MRNG_MOB_MOVE_COOLDOWN = 0.25
local MRNG_MOB_GC_SEC = 6
local MRNG_DUNGEON_FALLBACK_SCAN_SEC = 4
local MRNG_TT_AUTO_START_SEC = 12

local function mrngGetPlayerGui()
    return LP and LP:FindFirstChildOfClass("PlayerGui")
end

local function mrngHiddenMobFolder()
    local pg = mrngGetPlayerGui()
    if not pg then return nil end
    local f = pg:FindFirstChild(MRNG_HIDDEN_MOBS_GUI)
    if not f then
        f = Instance.new("Folder")
        f.Name = MRNG_HIDDEN_MOBS_GUI
        f.Parent = pg
    end
    return f
end

local function mrngDungeonFolder()
    return workspace:FindFirstChild("GeneratedDungeon")
end

local function mrngParseRoomNumber(v)
    local s = tostring(v or states.dungeonRoomInput or "1")
    local n = tonumber(s:match("%d+"))
    if not n then return 1 end
    return math.clamp(math.floor(n), 1, 1000000)
end

local function mrngRoomNumber(model)
    if not model then return nil end
    return tonumber(tostring(model.Name):match("^Room_(%d+)$"))
end

local function mrngDungeonFindRoom(num)
    local gd = mrngDungeonFolder()
    if not gd then return nil end
    return gd:FindFirstChild("Room_" .. tostring(mrngParseRoomNumber(num)))
end

local function mrngDungeonRememberRoom(room)
    if not (room and room:IsA("Model")) then return end
    local n = mrngRoomNumber(room)
    if n and n > (_mrngDungeonLatestN or -1) then
        _mrngDungeonLatestN = n
        _mrngDungeonLatestCached = room
    end
end

local function mrngDungeonRescanLatest()
    local gd = mrngDungeonFolder()
    if not gd then return nil end
    local best, bestN = nil, -1
    for _, ch in ipairs(gd:GetChildren()) do
        if ch:IsA("Model") then
            local n = mrngRoomNumber(ch)
            if n and n > bestN then bestN = n; best = ch end
        end
    end
    _mrngDungeonLatestCached = best
    _mrngDungeonLatestN = bestN
    return best, bestN
end

local function mrngDungeonLatestRoom()
    if _mrngDungeonLatestCached and _mrngDungeonLatestCached.Parent then
        return _mrngDungeonLatestCached, _mrngDungeonLatestN
    end
    return mrngDungeonRescanLatest()
end

local function mrngDungeonBase(room)
    if not (room and room.Parent) then return nil end
    local b = room:FindFirstChild("BASE") or room:FindFirstChild("BASE", true) or room.PrimaryPart
    if b and b:IsA("BasePart") then return b end
    return nil
end

local function mrngTpAboveBase(base)
    local root = getRoot()
    if not (root and base and base.Parent) then return false end
    local y = (base.Size and base.Size.Y or 0) / 2 + 1
    root.CFrame = base.CFrame * CFrame.new(0, y, 0)
    return true
end

local function mrngTpRoom(room)
    return mrngTpAboveBase(mrngDungeonBase(room))
end

local function mrngStartDungeonAuto()
    if _mrngDungeonAutoStarted then return end
    _mrngDungeonAutoStarted = true
    addThread(task.spawn(function()
        local lastGd = nil
        local lastFallbackScan = 0
        while GEN == _G.MeleeRNG_Gen do
            local gd = mrngDungeonFolder()
            if gd and gd ~= lastGd then
                lastGd = gd
                mrngDungeonRescanLatest()
                addConn(gd.ChildAdded:Connect(function(ch)
                    if GEN ~= _G.MeleeRNG_Gen then return end
                    if ch and ch:IsA("Model") then
                        mrngDungeonRememberRoom(ch)
                        if states.dungeonAutoTpNew then
                            task.defer(function()
                                local n = mrngRoomNumber(ch)
                                if n and _mrngDungeonLastTpRoom ~= n and mrngTpRoom(ch) then
                                    _mrngDungeonLastTpRoom = n
                                end
                            end)
                        end
                    end
            ...
            local now = os.clock()
            if gd and now - lastFallbackScan >= MRNG_DUNGEON_FALLBACK_SCAN_SEC then
                lastFallbackScan = now
                mrngDungeonRescanLatest()
            end

            -- No more scanning every 0.20s as Room_N grows into the hundreds/thousands.
            if states.dungeonAutoTpNew then
                local room, n = mrngDungeonLatestRoom()
                if room and n and _mrngDungeonLastTpRoom ~= n then
                    if mrngTpRoom(room) then _mrngDungeonLastTpRoom = n end
                end
            end
            if states.dungeonAutoTpEntered then
                local targetN = mrngParseRoomNumber(states.dungeonRoomInput)
                if _mrngDungeonLastEnteredTp ~= targetN or (now - _mrngDungeonLastEnteredAt) >= 1.25 then
                    local room = mrngDungeonFindRoom(targetN)
                    if room and mrngTpRoom(room) then
                        _mrngDungeonLastEnteredTp = targetN
                        _mrngDungeonLastEnteredAt = now
                    end
                end
            end
            task.wait(0.35)
...
local function mrngMoveMobToMe(model, force)
    if not (states.perfMobsSpawnOnMe and model and model:IsA("Model")) then return end
    local now = os.clock()
    -- Keep mobs close enough for touch registration, but throttle each model.
    -- The old one-move-only behavior let mobs drift/replicate away, which made one-shot unreliable.
    if force ~= true and _mrngMobLastMove[model] and (now - _mrngMobLastMove[model]) < MRNG_MOB_MOVE_COOLDOWN then return end
    _mrngMobLastMove[model] = now
    local root = getRoot()
    if not root then return end
    local mobRoot = model:FindFirstChild("HumanoidRootPart") or model.PrimaryPart
    if mobRoot and mobRoot:IsA("BasePart") then
        pcall(function()
            mobRoot.Anchored = false
            mobRoot.CFrame = root.CFrame * CFrame.new(0, 0, -4)
        end)
        pcall(function() model:SetAttribute("_MRNGMovedOnce", true) end)
    end
end

local function mrngHardHideMob(model, hide)
    if not (model and model:IsA("Model")) then return end
    if hide and model:GetAttribute("_MRNGHardHidden") == true then return end
    local stash = mrngHiddenMobFolder()
    local root = model:FindFirstChild("HumanoidRootPart") or model.PrimaryPart
    for _, d in ipairs(model:GetDescendants()) do
        if d:IsA("BasePart") and d ~= root then
            if hide then
                if _mrngMobPartParent[d] == nil then _mrngMobPartParent[d] = d.Parent end
                if stash then pcall(function() d.Parent = stash end) end
            else
                local old = _mrngMobPartParent[d]
                if old and old.Parent then pcall(function() d.Parent = old end) end
                _mrngMobPartParent[d] = nil
            end
        elseif d:IsA("ParticleEmitter") or d:IsA("Trail") or d:IsA("Beam") or d:IsA("PointLight") or d:IsA("SpotLight") or d:IsA("SurfaceLight") or d:IsA("Highlight") then
            if hide then
                if _mrngMobFxEnabled[d] == nil then _mrngMobFxEnabled[d] = d.Enabled end
                pcall(function() d.Enabled = false end)
            else
                local was = _mrngMobFxEnabled[d]
                if was ~= nil then pcall(function() d.Enabled = was end) end
                _mrngMobFxEnabled[d] = nil
            end
        end
    end
    pcall(function() model:SetAttribute("_MRNGHardHidden", hide == true) end)
end

local function mrngMobGcStaleHidden()
    -- Late-lag fix: when mob models despawn, their reparented client-only visual
    -- parts can stay in PlayerGui forever unless we purge them.
    for part, old in pairs(_mrngMobPartParent) do
        if not part or not part.Parent then
            _mrngMobPartParent[part] = nil
        elseif not old or not old.Parent then
            pcall(function() part:Destroy() end)
            _mrngMobPartParent[part] = nil
        end
    end
    for fx in pairs(_mrngMobFxEnabled) do
        if not fx or not fx.Parent then
            _mrngMobFxEnabled[fx] = nil
        end
    end
    for model in pairs(_mrngMobLastMove) do
        if not model or not model.Parent then
            _mrngMobLastMove[model] = nil
        end
    end
end

local function mrngSweepMobsNow(limit, forceMove)
    local mobs = workspace:FindFirstChild("Mobs")
    if not mobs then return end
    local cap = tonumber(limit) or math.huge
    local done = 0
    for _, m in ipairs(mobs:GetChildren()) do
        if done >= cap then break end
        if m:IsA("Model") then
            mrngMoveMobToMe(m, forceMove == true)
            if states.perfHardHideMobs then mrngHardHideMob(m, true) end
            done += 1
        end
    end
end

local function mrngRestoreHardHiddenMobs()
    for part, old in pairs(_mrngMobPartParent) do
        pcall(function()
            if part and old and old.Parent then part.Parent = old end
        end)
        _mrngMobPartParent[part] = nil
    end
    for fx, was in pairs(_mrngMobFxEnabled) do
        pcall(function() if fx and fx.Parent then fx.Enabled = was end end)
        _mrngMobFxEnabled[fx] = nil
    end
end

local function mrngStartMobPerf()
    if _mrngMobPerfStarted then return end
    _mrngMobPerfStarted = true
    addThread(task.spawn(function()
        local lastMobs = nil
        local lastSweep = 0
        local lastGc = 0
        while GEN == _G.MeleeRNG_Gen do
            local mobs = workspace:FindFirstChild("Mobs")
            if mobs and mobs ~= lastMobs then
                lastMobs = mobs
                addConn(mobs.ChildAdded:Connect(function(ch)
                    if GEN ~= _G.MeleeRNG_Gen then return end
                    task.defer(function()
                        if ch and ch:IsA("Model") then
                            mrngMoveMobToMe(ch, true)
                            if states.perfHardHideMobs then mrngHardHideMob(ch, true) end
                        end
                    end)
                end))
            end

            if states.perfMobsSpawnOnMe or states.perfHardHideMobs then
                local now = os.clock()
                if now - lastGc >= MRNG_MOB_GC_SEC then
                    lastGc = now
                    mrngMobGcStaleHidden()
                end
                if now - lastSweep >= MRNG_MOB_SWEEP_SEC then
                    lastSweep = now
                    -- Capped slow pass.  This is the big lag fix: no more full
                    -- workspace.Mobs:GetChildren()+GetDescendants sweeps every 0.35s.
                    mrngSweepMobsNow(MRNG_MOB_SWEEP_LIMIT, false)
                end
                task.wait(0.25)
            else
                if next(_mrngMobPartParent) ~= nil or next(_mrngMobFxEnabled) ~= nil then
                    mrngRestoreHardHiddenMobs()
                end
                task.wait(0.9)
            end
        end
...
local function mrngRemoteCall(name, ...)
    local r = R(name)
    ...
    return ok, ok and "fired" or err
end

local function mrngTTParseKills(...)
    local args = table.pack(...)
    local a, b = args[1], args[2]
    if type(a) == "table" then
        local cur = a.CurrentKills or a.Current or a.Kills or a.kills or a[1]
        local req = a.RequiredKills or a.Required or a.Needed or a.Goal or a.required or a[2]
        _mrngTTCurrent = tonumber(cur) or _mrngTTCurrent
        _mrngTTRequired = tonumber(req) or tonumber(cur) or _mrngTTRequired
    elseif tonumber(a) and tonumber(b) then
        _mrngTTCurrent = tonumber(a)
        _mrngTTRequired = tonumber(b)
    elseif tonumber(a) then
        _mrngTTRequired = tonumber(a)
    end
end

local function mrngTTStatusText()
    local cur = _mrngTTCurrent and tostring(_mrngTTCurrent) or "?"
    local req = _mrngTTRequired and tostring(_mrngTTRequired) or "?"
    local tl  = _mrngTTTimeLeft and tostring(_mrngTTTimeLeft) or "?"
    return "Time Trial: kills " .. cur .. " / " .. req .. " | time left " .. tl
end

local function mrngTTRefreshInfo()
    local ok1, timeLeft = mrngRemoteCall("GetUpdateTimeLeft")
    if ok1 and timeLeft ~= nil then _mrngTTTimeLeft = timeLeft end
    local ok2, found = mrngRemoteCall("GetTTFoundCount")
    if ok2 and tonumber(found) then _mrngTTCurrent = tonumber(found) end
    return mrngTTStatusText()
end

local function mrngStartTimeTrialWatch(statusLbl)
    if _mrngTTStarted then return end
    _mrngTTStarted = true
    local u = R("UpdateTimeTrialKills")
    if u and (u:IsA("RemoteEvent") or u.ClassName == "UnreliableRemoteEvent") then
        addConn(u.OnClientEvent:Connect(function(...)
            mrngTTParseKills(...)
            if statusLbl and statusLbl.Set then statusLbl.Set(mrngTTStatusText()) end
        end))
    end
    addThread(task.spawn(function()
        local lastStart = 0
        while GEN == _G.MeleeRNG_Gen do
            ...
            if states.timeTrialAutoStart then
                local now = os.clock()
                if now - lastStart >= MRNG_TT_AUTO_START_SEC then
                    lastStart = now
                    mrngRemoteCall("StartTimeTrial")
                end
            end
        end
    end))
...
local function meleeMountGeneratedDungeonSection(MiscTab, SLbl, C)
    mrngStartDungeonAuto()
    MiscTab:Section("GeneratedDungeon")
    MiscTab:Label("Uses workspace.GeneratedDungeon.Room_N and the BASE part/PrimaryPart. TP lands 1 stud above BASE.")
    MiscTab:Toggle("Auto TP new rooms", "Whenever a higher Room_N appears, teleport to BASE + 1 stud.", states.dungeonAutoTpNew, function(on)
        states.dungeonAutoTpNew = on == true
        saveSettings()
        SLbl.Text = states.dungeonAutoTpNew and "GeneratedDungeon auto TP ON" or "GeneratedDungeon auto TP OFF"
    end)
    MiscTab:Input("Room number", "Enter 1, 2, 958, or Room_958. Saved.", "Room_N", tostring(states.dungeonRoomInput or "1"), function(txt)
        states.dungeonRoomInput = tostring(txt or "1")
        saveSettings()
        SLbl.Text = "Target room = Room_" .. tostring(mrngParseRoomNumber(states.dungeonRoomInput))
    end)
    MiscTab:Toggle("Auto TP entered Room_N", "Keeps trying to teleport to the typed Room_N once it exists.", states.dungeonAutoTpEntered, function(on)
        states.dungeonAutoTpEntered = on == true
        saveSettings()
        SLbl.Text = states.dungeonAutoTpEntered and "Auto target Room_N ON" or "Auto target Room_N OFF"
    end)
    MiscTab:Button("🎯", "TP entered Room_N now", "Teleports to the typed server-visible room if it exists.", C.accent, function()
        local n = mrngParseRoomNumber(states.dungeonRoomInput)
        local room = mrngDungeonFindRoom(n)
        SLbl.Text = (room and mrngTpRoom(room)) and ("TP → Room_" .. tostring(n)) or ("Room_" .. tostring(n) .. " not found / no BASE")
    end)
    MiscTab:Button("⏩", "TP latest room now", "Finds the highest generated Room_N and teleports to BASE.", C.green, function()
        local room, n = mrngDungeonLatestRoom()
        SLbl.Text = (room and mrngTpRoom(room)) and ("TP → latest Room_" .. tostring(n)) or "No generated room with BASE found"
    end)
end

local function meleeMountExtraAntiLagSection(MiscTab, SLbl, C)
    mrngStartMobPerf()
    MiscTab:Section("Extra Anti-Lag / Mobs")
    MiscTab:Slider("Auto Equip Best interval", "Raise this if EquipBest causes a hitch. Saved.",
        {min=10, max=300, default=math.clamp(math.floor(tonumber(states.autoEquipBestInterval) or 45), 10, 300), format=function(v) return tostring(math.floor(v)) .. "s" end},
        function(v)
            states.autoEquipBestInterval = math.clamp(math.floor(tonumber(v) or 45), 10, 300)
            saveSettings()
            SLbl.Text = "Auto Equip Best interval = " .. tostring(states.autoEquipBestInterval) .. "s"
        end)
    MiscTab:Toggle("Mobs spawn on me", "Moves workspace.Mobs models to your HumanoidRootPart as they appear.", states.perfMobsSpawnOnMe, function(on)
        states.perfMobsSpawnOnMe = on == true
        saveSettings()
        if states.perfMobsSpawnOnMe then mrngSweepMobsNow(MRNG_MOB_SWEEP_LIMIT, true) end
        SLbl.Text = states.perfMobsSpawnOnMe and "Mob move-to-me ON" or "Mob move-to-me OFF"
    end)
    MiscTab:Button("📍", "Move all mobs to me now", "One-shot sweep of workspace.Mobs roots to your character.", C.accent, function()
        local old = states.perfMobsSpawnOnMe
        states.perfMobsSpawnOnMe = true
        mrngSweepMobsNow(math.huge, true)
        states.perfMobsSpawnOnMe = old
        SLbl.Text = "Moved current mobs to you"
    end)
    MiscTab:Toggle("Hard-hide mob models", "Stashes non-root mob parts/effects in PlayerGui while keeping HumanoidRootPart active.", states.perfHardHideMobs, function(on)
        states.perfHardHideMobs = on == true
        saveSettings()
        if states.perfHardHideMobs then mrngSweepMobsNow(MRNG_MOB_SWEEP_LIMIT, false) else mrngRestoreHardHiddenMobs() end
        SLbl.Text = states.perfHardHideMobs and "Hard-hide mobs ON" or "Hard-hide mobs OFF"
    end)
end

local function meleeMountTimeTrialsSection(MiscTab, SLbl, C)
    MiscTab:Section("Time Trials")
    local ttLbl = MiscTab:Label(mrngTTStatusText())
    mrngStartTimeTrialWatch(ttLbl)
    MiscTab:Toggle("⏱️ Auto Start Time Trial", "Periodically calls the existing StartTimeTrial remote while ON.", states.timeTrialAutoStart, function(on)
        states.timeTrialAutoStart = on == true
        saveSettings()
        SLbl.Text = states.timeTrialAutoStart and "Time Trial auto start ON" or "Time Trial auto start OFF"
    end)
    MiscTab:Button("▶️", "Start Time Trial once", "Calls StartTimeTrial one time.", C.green, function()
        local ok, res = mrngRemoteCall("StartTimeTrial")
        SLbl.Text = ok and "StartTimeTrial sent" or ("StartTimeTrial failed: " .. tostring(res))
    end)
    MiscTab:Button("🔄", "Refresh Time Trial info", "Reads GetUpdateTimeLeft / GetTTFoundCount when available.", C.accent, function()
        ttLbl.Set(mrngTTRefreshInfo())
        SLbl.Text = "Time Trial info refreshed"
    end)
    MiscTab:Button("🧪", "Print Time Trial remotes", "Prints class names for the Time Trial remotes shown in your screenshots.", C.sub, function()
        local names = {"StartTimeTrial", "UpdateTimeTrialKills", "GetUpdateTimeLeft", "GetPossibleSpawnPositionsForTimeTrial", "GetTTFoundCount", "SpawnMob", "UpdateTimeTrialLobby"}
        local rf = RS:FindFirstChild("Remotes")
        for _, name in ipairs(names) do
            local r = rf and rf:FindFirstChild(name)
            print("[MeleeRNG/TT]", name, r and r.ClassName or "MISSING")
        end
        SLbl.Text = "Printed Time Trial remotes to console"
    end)
end
...
local HITBOX_BATCH_MAX   = 24 -- one-shot guard: enough sword hitboxes resize per pulse without old all-sword lag
local HITBOX_HOLD_FRAMES = 2  -- keep BIG briefly so Touched has time to register under load
...
hitboxDutySlider = MoveTab:Slider("Big phase duty (1 big / N heartbeats)", "One frame BIG per cycle, rest tiny. N=2 ≈ half the Heartbeats BIG. Higher N = calmer physics, fewer Touched edges.",
    {min=3, max=HITBOX_DUTY_MAX, default=math.clamp(math.floor(tonumber(states.hitboxDutyCycle) or 4), 3, HITBOX_DUTY_MAX),
     format=function(v) return "1 big / "..tostring(math.floor(v)) end},
    function(v)
        states.hitboxDutyCycle = math.clamp(math.floor(v), 3, HITBOX_DUTY_MAX)
        saveSettings()
    end)
...
local function applyHitboxPulsePreset(title, duty, radius)
    local d = math.clamp(math.floor(duty), 3, HITBOX_DUTY_MAX)
...
MoveTab:Button("⚡", "Max farm — duty 3", "BIG 500 · strongest stable flip rate + map-tuned reach", C.green, function()
    applyHitboxPulsePreset("Max farm — duty 3", 3, HITBOX_BIG_DEFAULT)
end)
...
addThread(task.spawn(function()
    while true do
        ...
        -- Do not sweep weapon folders every Heartbeat. Hidden parts still cost scan time;
        -- throttle this loop so Hide Weapons reduces lag instead of causing it.
        task.wait((ow or sw or (hv and not sw)) and 0.18 or 0.12)
...
local function setHitboxPulse(on)
    ...
    if on then
        ...
        _pulseConn = RunService.Heartbeat:Connect(function()
            ...
            -- Duty cycle: BIG only 1 frame per N.  Old code resized every sword every
            -- Heartbeat; this rolling batch keeps Touched behavior without weapon-count lag.
            _dutyTick = _dutyTick + 1
            local duty = math.clamp(math.floor(tonumber(states.hitboxDutyCycle) or 4), 3, HITBOX_DUTY_MAX)
            local hCache = _hitboxCache
            local nh = #hCache

            local batches = _G.MeleeRNG_HitboxBigBatches
            if type(batches) ~= "table" then batches = {}; _G.MeleeRNG_HitboxBigBatches = batches end
            for bi = #batches, 1, -1 do
                local b = batches[bi]
                b.hold = (tonumber(b.hold) or 0) - 1
                if b.hold <= 0 then
                    local parts = b.parts
                    ...
                    table.remove(batches, bi)
                end
            end

            _pulsePhase = ((_dutyTick - 1) % duty) == 0
            if _pulsePhase and nh > 0 then
                local batch = math.min(nh, HITBOX_BATCH_MAX)
                local cursor = math.floor(tonumber(_G.MeleeRNG_HitboxCursor) or 1)
                local activeBatch = {}
                for _ = 1, batch do
                    if cursor > nh then cursor = 1 end
                    local hb = hCache[cursor]
                    if hb and hb.Parent then
                        if hb.Size ~= _pulseBigVec then hb.Size = _pulseBigVec end
                        activeBatch[#activeBatch + 1] = hb
                    else
                        _hitboxCacheDirty = true
                    end
                    cursor = cursor + 1
                end
                _G.MeleeRNG_HitboxCursor = cursor
                table.insert(_G.MeleeRNG_HitboxBigBatches, { hold = HITBOX_HOLD_FRAMES, parts = activeBatch })
            end
        end)
        addConn(_pulseConn)
        rebuildHitboxCache()
        SLbl.Text = string.format("⚡ Pulse ON | %.0f studs | %d hitboxes | batch %d | hold %d | duty 1/%d", hitboxSize, #_hitboxCache, HITBOX_BATCH_MAX, HITBOX_HOLD_FRAMES, math.clamp(math.floor(tonumber(states.hitboxDutyCycle) or 4), 3, HITBOX_DUTY_MAX))
    else
        if _pulseConn then _pulseConn:Disconnect(); _pulseConn = nil end
        ...
        _pulseLastBig = {}
        _pulseCursor = 1
        _G.MeleeRNG_HitboxBigBatches = nil
        _G.MeleeRNG_HitboxCursor = nil
...
local autoEquipBestToggle = MoveTab:Toggle("⚔️ Auto Equip Best", "Remotes.EquipBest:FireServer() on a saved interval while ON — higher interval = fewer weapon rebuild spikes", states.autoEquipBest, function(on)
...
addThread(task.spawn(function()
    while true do
        if GEN ~= _G.MeleeRNG_Gen then break end
        task.wait(math.clamp(math.floor(tonumber(states.autoEquipBestInterval) or 45), 10, 300))
...
-- Keep these stable add-ons at the bottom of Misc.  Time Trials is intentionally last.
meleeMountGeneratedDungeonSection(MiscTab, SLbl, C)
meleeMountExtraAntiLagSection(MiscTab, SLbl, C)
meleeMountTimeTrialsSection(MiscTab, SLbl, C)
