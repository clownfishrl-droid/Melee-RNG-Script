...
-- ── Time Trials lobby Start Run auto-click ─────────────────────
-- The StartTimeTrial remote can open the game's Time Trials Lobby UI first.
-- This helper waits for that GUI and clicks the visible "Start Run" button.
...
local function meleeTextFromGuiButton(btn)
    local text = ""
    pcall(function()
        if btn:IsA("TextButton") then text = tostring(btn.Text or "") end
    end)
    if text == "" then
        pcall(function()
...
local function meleeFindTimeTrialStartRunButton()
    ...
    -- Exact path from the live UI: PlayerGui.TimeTrialsUI.StartBtn
    local ttGui = pg:FindFirstChild("TimeTrialsUI")
    local direct = ttGui and ttGui:FindFirstChild("StartBtn", true)
    if direct and (direct:IsA("TextButton") or direct:IsA("ImageButton")) and meleeGuiIsActuallyVisible(direct) then
        return direct
    end
    ...
    for _, d in ipairs(pg:GetDescendants()) do
        if d:IsA("TextButton") or d:IsA("ImageButton") then
            local text = meleeTextFromGuiButton(d)
            ...
            if isStartRun and meleeGuiIsActuallyVisible(d) then
                ...
                while p and p ~= pg do
                    ...
                    if pn:find("timetrial") or pn:find("time") or pn:find("lobby") then score = score + 15 end
                    if pn:find("nexus") or pn:find("meleerng") then score = score - 40 end
                    p = p.Parent
                end
                if score > bestScore then
...
function meleeTimeTrialClickStartRun(reason)
    local btn = meleeFindTimeTrialStartRunButton()
    if not btn then return false end
    local clicked = fireGuiButtonClick(btn)
    if not clicked then
        local fs = rawget(_G, "firesignal")
    ...
    if clicked then
        vprint("[MeleeRNG/TimeTrial] clicked Start Run", tostring(reason or ""))
    end
    return clicked == true
end
...
local function meleeFindTimeTrialLeaveButton()
    ...
    -- Exact path from the live UI: PlayerGui.MainGUI.GeneralUI.LeaveTimeTrialBtn
    local main = pg:FindFirstChild("MainGUI")
    local general = main and main:FindFirstChild("GeneralUI")
    ...
    for _, d in ipairs(pg:GetDescendants()) do
        if d:IsA("TextButton") or d:IsA("ImageButton") then
            ...
            if isLeave and meleeGuiIsActuallyVisible(d) then
                ...
                while p and p ~= pg do
                    local pn = tostring(p.Name):lower()
                    if pn:find("timetrial") or pn:find("time") or pn:find("generalui") then score = score + 15 end
                    if pn:find("nexus") or pn:find("meleerng") then score = score - 40 end
                    p = p.Parent
                end
                if score > bestScore then
...
function meleeTimeTrialClickLeave(reason)
    local btn = meleeFindTimeTrialLeaveButton()
    if not btn then return false end
    local clicked = fireGuiButtonClick(btn)
    if not clicked then
        local fs = rawget(_G, "firesignal")
    ...
    if clicked then
        vprint("[MeleeRNG/TimeTrial] clicked Leave", tostring(reason or ""))
    end
    return clicked == true
end
    ...
    task.spawn(function()
        while GEN == _G.MeleeRNG_Gen and os.clock() < _ttStartRunBurstUntil do
            if meleeTimeTrialClickStartRun(reason or "burst") then
                ...
                task.wait(tonumber(interval) or 0.18)
            end
        end
...
function meleeMountSafeDungeonMobTimeTrialAddons(MiscTab, SLbl, C)
    ...
    addThread(task.spawn(function()
        ...
        while true do
            ...
            task.wait((states.mobSpawnOnMe or states.mobHardHide) and 0.35 or 1.5)
        end
    end))
    ...
    MiscTab:Button("▶️", "Start Time Trial once", "Calls the existing StartTimeTrial remote once, then auto-clicks Start Run if the lobby opens.", C.green, function()
        ...
        if states.timeTrialAutoClickStartRun then
            meleeTimeTrialClickStartRunBurst(8, 0.18, "start once")
        end
    ...
    addThread(task.spawn(function()
        while true do
            ...
            if states.timeTrialAutoStart then
                ...
                if states.timeTrialAutoClickStartRun then
                    meleeTimeTrialClickStartRunBurst(8, 0.18, "auto loop")
                end
                task.wait(6)
            else
                task.wait(1)
                ...
                if room and room ~= lastLeaveModel and os.clock() >= nextLeaveAllowed and meleeFindTimeTrialLeaveButton() then
                    ...
                    if meleeTimeTrialClickLeave("auto room target") then
                        task.delay(1.25, function()
                            ...
                            pcall(function() fireR("StartTimeTrial") end)
                            if states.timeTrialAutoClickStartRun then
                                meleeTimeTrialClickStartRunBurst(9, 0.18, "auto restart after leave")
                            end
                        end)
                    end
                end
