...
-- ── Old GUI MP4 / animated background ─────────────────────────
-- This keeps the original NexusLib UI. First it tries the real MP4 through
-- VideoFrame. If Roblox / the executor refuses local MP4 video playback, it
-- falls back to an embedded animated frame version of the same MP4 so the
-- background is actually visible instead of turning into a blank gray panel.
...
local function meleeApplyOldGuiMp4Background(enabled)
    ...
    -- Use a real VideoFrame only when a Roblox video asset id is provided.
    -- Local/GitHub MP4s often return a custom asset string but still render blank,
    -- which is exactly why the older build looked gray. For local/GitHub use, the
    -- embedded animated frames are the visible fallback and are shown instead.
    local src = uiBgGetVideoSource()
    local useRealRobloxVideo = (type(UI_BG_ASSET_ID) == "string" and UI_BG_ASSET_ID ~= "" and src ~= nil)
    ...
    if useRealRobloxVideo then
        ...
        video.Visible = false
        local assets = uiBgEnsureEmbeddedFrames()
        if assets and #assets > 0 then
            uiBgStartFrameAnimation(img, assets)
            UI_BG_LAST_STATUS = "using visible animated MP4 fallback frames"
        end
    end
...
MiscTab:Label("Old GUI kept. Background now uses the real MP4 when Roblox/executor allows it, and an embedded animated frame fallback when VideoFrame refuses local MP4. It fits/crops behind the NexusLib window.")
