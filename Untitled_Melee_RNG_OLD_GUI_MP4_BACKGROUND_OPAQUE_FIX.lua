...
local states = {
    ...
    uiVideoBackground = true, -- old GUI only: MP4 background behind NexusLib window
...
-- ── Old GUI MP4 background ───────────────────────────────────
-- This keeps the original NexusLib UI layout/style. It only adds a VideoFrame
-- behind the window. The video is fully opaque, then a dark readability layer
-- and mostly-solid old GUI panels sit above it so the UI is not see-through.
local UI_BG_VIDEO_FILE = "MeleeRNG_UI_Background.mp4"
-- GitHub raw MP4 fallback. Upload MeleeRNG_UI_Background.mp4 next to the Lua in your repo,
-- or replace this with your own raw MP4 URL.
...
local UI_BG_ASSET_ID = "" -- Optional: Roblox video asset id, if you upload the MP4 to Roblox.
local UI_BG_LAST_STATUS = "not checked"
local UI_BG_SCREEN_ATTR = "MeleeRNG_OldGuiMp4Background"
local UI_BG_VIDEO_NAME = "MeleeRNG_OldGui_MP4_Background"
local UI_BG_DIM_NAME = "MeleeRNG_OldGui_MP4_Dim"
local UI_BG_PANEL_ALPHA_ATTR = "MeleeRNG_OldGuiMp4PanelAlphaFixed"
-- Lower = more solid. 0.10 means old panels are only 10% see-through.
local UI_BG_PANEL_TRANSPARENCY = 0.10
-- Higher = less black over the video. Keep this around 0.32-0.45 for readability.
local UI_BG_DIM_TRANSPARENCY = 0.34
local _uiBgRoot = nil
...
local function uiBgIsOwnObject(obj)
    return obj and (obj.Name == UI_BG_VIDEO_NAME or obj.Name == UI_BG_DIM_NAME)
end
...
local function uiBgSoftenLargePanel(obj)
    ...
    if sx < 260 or sy < 150 then return end

    -- Previous MP4 build made these large panels too transparent. Keep the
    -- old GUI readable/solid while still letting the MP4 show as the backdrop.
    obj:SetAttribute(UI_BG_SCREEN_ATTR, true)
    obj:SetAttribute(UI_BG_PANEL_ALPHA_ATTR, UI_BG_PANEL_TRANSPARENCY)
    obj.BackgroundColor3 = obj.BackgroundColor3:Lerp(Color3.fromRGB(10, 13, 24), 0.28)
    obj.BackgroundTransparency = math.min(math.max(obj.BackgroundTransparency, 0), UI_BG_PANEL_TRANSPARENCY)
...
local function meleeApplyOldGuiMp4Background(enabled)
    ...
    local oldVideo = root:FindFirstChild(UI_BG_VIDEO_NAME)
    local oldDim = root:FindFirstChild(UI_BG_DIM_NAME)
    ...
    if not dim then
        ...
        dim.BackgroundTransparency = src and UI_BG_DIM_TRANSPARENCY or 1
...
local function uiBgStatusText()
    return states.uiVideoBackground and ("MP4 background: ON — " .. tostring(UI_BG_LAST_STATUS)) or "MP4 background: OFF"
end
...
-- --------------------------------------------------------------------------
local Win  = Lib:Window({ title = "⚔️  Melee RNG  v1.1.2 Target Upgrades + MP4 BG" })
...
MiscTab:Section("MP4 Background")
MiscTab:Label("Old GUI kept. This only adds your MP4 behind the NexusLib window with an opaque video layer + dark readability overlay, so the UI is not clear/see-through. Upload MeleeRNG_UI_Background.mp4 beside this Lua on GitHub, or keep it in the executor workspace.")
local uiBgStatusLbl = MiscTab:Label(uiBgStatusText())
MiscTab:Toggle("🎥 GUI MP4 Background", "ON = use MeleeRNG_UI_Background.mp4 as the old GUI background. If missing locally, it tries the GitHub raw URL near the top of the script.", states.uiVideoBackground, function(on)
...
MiscTab:Button("⬇️", "Download / Refresh MP4", "Force-downloads the MP4 from UI_BG_GITHUB_RAW_URL, then reapplies the old-GUI background.", C.accent, function()
    local ok = uiBgEnsureVideoDownloaded(true)
    meleeApplyOldGuiMp4Background(states.uiVideoBackground)
    if uiBgStatusLbl and uiBgStatusLbl.Set then uiBgStatusLbl.Set(uiBgStatusText()) end
