...
local states = {
    ...
    timeTrialLeaveAtRoom = 0, -- Time Trial room threshold for auto leave/restart; 0 = disabled/no exact target
...
states.timeTrialLeaveAtRoom = math.clamp(math.floor(tonumber(states.timeTrialLeaveAtRoom) or 0), 0, 1000000)
...
local function meleeTTObjectPathAllowed(obj, mode)
    if not obj or not obj:IsA("GuiObject") then return false end
    local pg = LP and LP:FindFirstChildOfClass("PlayerGui")
    ...
    if mode == "leave" then
        local leave = general:FindFirstChild("LeaveTimeTrialBtn")
        return leave and (obj == leave or obj:IsDescendantOf(leave))
    end

    local lobby = general:FindFirstChild("TimeTrialsLobbyFrame")
    local startBtn = lobby and lobby:FindFirstChild("StartBtn")
    if startBtn and (obj == startBtn or obj:IsDescendantOf(startBtn)) then
        return true
    end

    -- Optional older-clone fallback, still scoped to TimeTrialsUI.StartBtn only.
    local ttGui = pg:FindFirstChild("TimeTrialsUI")
    local oldStart = ttGui and ttGui:FindFirstChild("StartBtn", true)
    return oldStart and (obj == oldStart or obj:IsDescendantOf(oldStart)) or false
end

local function meleeTTClickModeFromReason(reason)
    local r = tostring(reason or ""):lower()
    if r:find("leave", 1, true) then return "leave" end
    return "start"
end
...
local function meleeRobustClickGuiTarget(obj, reason)
    ...
    local mode = meleeTTClickModeFromReason(reason)
    if not meleeTTObjectPathAllowed(obj, mode) then
        vprint("[MeleeRNG/TimeTrial] blocked off-path click", tostring(reason or ""), obj:GetFullName())
        return false
    end

    local candidates, seen = {}, {}
    local function addCandidate(x)
        if x and x:IsA("GuiObject") and not seen[x] and meleeGuiIsActuallyVisible(x) and meleeTTObjectPathAllowed(x, mode) then
            seen[x] = true
            candidates[#candidates + 1] = x
        end
    end

    -- Exact object first, then only actual button descendants inside that exact path.
    -- No global PlayerGui text search, no parent-wrapper clicking, no unrelated UI buttons.
    addCandidate(obj)
    local directButton = meleeFindClickableDescendant(obj)
    if directButton then addCandidate(directButton) end
    ...
    for _, target in ipairs(candidates) do
        ...
        -- Center-click is allowed only on the verified exact TimeTrial button path.
        -- This prevents clicking random world/UI buttons when a fallback search is wrong.
        clickedCenter = meleeVirtualClickGuiObject(target, 1) or clickedCenter
        task.wait(0.10)
    end

    if signaled or clickedCenter then
        vprint("[MeleeRNG/TimeTrial] clicked exact path", tostring(obj:GetFullName()), tostring(reason or ""), "candidates", #candidates)
        return true
    end
    return false
end
...
local function meleeFindTimeTrialStartRunButton()
    ...
    -- Scoped legacy fallback only; never scan all PlayerGui for text like "Start".
    local ttGui = pg:FindFirstChild("TimeTrialsUI")
...
local function meleeFindTimeTrialLeaveButton()
    ...
    -- Scoped fallback only under GeneralUI.
...
function meleeMountSafeDungeonMobTimeTrialAddons(MiscTab, SLbl, C)
    ...
    local function tpToBase(base)
        ...
        local dest = base.Position + Vector3.new(0, y, 0)
        -- Do not keep writing CFrame if we are already basically there.
        -- Repeated CFrame writes were the last big lag spike when rooms/waves advanced fast.
        if (r.Position - dest).Magnitude <= 6 then
            return true
        end
    ...
    -- Smart room TP: when rooms appear quickly, only TP once to the newest room after the burst settles.
    -- The old behavior teleported to every Room_N as it was created, which caused late/run-end lag.
    local pendingAutoRoom = nil
    local autoRoomTpWorker = false
    local lastAutoRoomTpAt = 0
    local AUTO_ROOM_TP_SETTLE_SEC = 0.28
    local AUTO_ROOM_TP_COOLDOWN_SEC = 0.90
    local AUTO_ROOM_TP_MAX_BASE_WAIT_SEC = 2.0

    local function setPendingAutoRoom(room)
        if not (room and room.Parent and roomIndex(room)) then return end
        local currentN = roomIndex(pendingAutoRoom) or -1
        local newN = roomIndex(room) or -1
        if newN >= currentN then
            pendingAutoRoom = room
        end
    end

    local function startAutoRoomTpWorker()
        if autoRoomTpWorker then return end
        autoRoomTpWorker = true
        task.spawn(function()
            while GEN == _G.MeleeRNG_Gen and states.dungeonAutoTpNew do
                local room = pendingAutoRoom
                pendingAutoRoom = nil
                if not room then break end

                -- Let fast room creation settle, then use only the newest pending room.
                task.wait(AUTO_ROOM_TP_SETTLE_SEC)
                local shouldTryTp = pendingAutoRoom == nil

                if shouldTryTp then
                    local remaining = AUTO_ROOM_TP_COOLDOWN_SEC - (os.clock() - lastAutoRoomTpAt)
                    if remaining > 0 then
                        task.wait(remaining)
                        shouldTryTp = pendingAutoRoom == nil
                    end
                end

                if shouldTryTp then
                    if not (room and room.Parent) then
                        local newest = latestRoom()
                        if newest then room = newest end
                    end

                    local deadline = os.clock() + AUTO_ROOM_TP_MAX_BASE_WAIT_SEC
                    while GEN == _G.MeleeRNG_Gen and states.dungeonAutoTpNew and room and room.Parent and os.clock() < deadline do
                        if pendingAutoRoom then
                            break
                        end
                        local b = roomBase(room)
                        if b and tpToBase(b) then
                            lastAutoRoomTpAt = os.clock()
                            updateDungeonStatus("Smart TP latest " .. room.Name .. " BASE +1")
                            break
                        end
                        task.wait(0.08)
                    end
            ...
            autoRoomTpWorker = false
            if GEN == _G.MeleeRNG_Gen and states.dungeonAutoTpNew and pendingAutoRoom then
                startAutoRoomTpWorker()
            end
        end)
    end
    ...
    local function onDungeonChild(ch)
        ...
        setPendingAutoRoom(ch)
        startAutoRoomTpWorker()
    end

    MiscTab:Section("GeneratedDungeon")
    dungeonStatus = MiscTab:Label("Dungeon room target: " .. roomName(states.dungeonTargetRoomNumber))
    MiscTab:Toggle("Auto TP new rooms", "Smart mode: waits for room bursts to settle, then TP only to the newest Room_N BASE +1.", states.dungeonAutoTpNew, function(on)
        states.dungeonAutoTpNew = on == true
        if not states.dungeonAutoTpNew then pendingAutoRoom = nil end
        saveSettings()
        updateDungeonStatus(states.dungeonAutoTpNew and "Smart auto TP new rooms ON" or "Auto TP new rooms OFF")
    end)
    ...
    -- Mob anti-lag: move roots immediately, then STASH visual parts out of Workspace.
    -- LocalTransparencyModifier alone still leaves hundreds/thousands of mob parts in Workspace;
    -- stashing non-root visuals removes render + physics overhead while keeping HumanoidRootPart alive.
    local mobHidden = {}
    local mobFx = {}
    local mobVisualParent = {}
    local mobQueue = {}
    local mobQueued = {}
    local mobProcessed = 0
    local mobLastCleanup = 0
    local mobNextStatus = 0
    local MOB_QUEUE_BATCH = 60      -- visual-stash batch only; mob movement is immediate
    local MOB_DESC_BUDGET = 90      -- yield less often; each mob should be hidden quickly
    local MOB_BACKUP_SWEEP_SEC = 4.0

    local function hiddenMobFolder()
        local pg = LP and LP:FindFirstChildOfClass("PlayerGui")
        if not pg then return nil end
        local f = pg:FindFirstChild("MeleeRNG_HiddenMobVisuals")
        if not f then
            f = Instance.new("Folder")
            f.Name = "MeleeRNG_HiddenMobVisuals"
            f.Parent = pg
        end
        return f
    end
    ...
    local function applyMobHardHide(model, hide)
        ...
        local stash = hide and hiddenMobFolder() or nil
        local budget = 0
        local desc = model:GetDescendants()
        for _, d in ipairs(desc) do
            if d:IsA("BasePart") then
                if d == root then
                    -- Keep the one part combat/targeting code needs, but make it cheap to render.
                    if hide then
                        ...
                        if old ~= nil then d.LocalTransparencyModifier = old; mobHidden[d] = nil end
                    end
                else
                    if hide then
                        if mobHidden[d] == nil then mobHidden[d] = d.LocalTransparencyModifier end
                        ...
                        pcall(function() d.CastShadow = false end)
                        pcall(function() d.CanCollide = false end)
                        -- Main lag fix: remove visual mob parts from Workspace entirely.
                        if stash and d.Parent and not mobVisualParent[d] then
                            mobVisualParent[d] = d.Parent
                            pcall(function() d.Parent = stash end)
                        end
                    else
                        local originalParent = mobVisualParent[d]
                        if originalParent and originalParent.Parent and d.Parent then
                            pcall(function() d.Parent = originalParent end)
                        end
                        mobVisualParent[d] = nil
                        local old = mobHidden[d]
                        if old ~= nil then
            ...
            budget = budget + 1
            if hide and budget >= MOB_DESC_BUDGET then
                budget = 0
                task.wait()
            end
        end
    ...
    local function moveMobToMe(model)
        ...
        if r and mr and model and model.Parent then
            pcall(function()
                -- Tiny spread prevents every root from occupying the exact same CFrame.
                local jitter = Vector3.new(math.random(-4, 4) * 0.08, 0, math.random(-4, 4) * 0.08)
                model:PivotTo(r.CFrame * CFrame.new(jitter.X, 0, -3 + jitter.Z))
            end)
        end
    end

    local ownWeaponForceNext = 0
    local function forceOwnWeaponHideSoon()
        if not states.perfHideOwnWeapons then return end
        local now = os.clock()
        if now < ownWeaponForceNext then return end
        ownWeaponForceNext = now + 0.08
        local fn = rawget(_G, "meleeForceHideOwnWeaponVisualsNow") or meleeForceHideOwnWeaponVisualsNow
        if type(fn) ~= "function" then return end
        pcall(fn)
        task.delay(0.03, function()
            if GEN == _G.MeleeRNG_Gen and states.perfHideOwnWeapons then pcall(fn) end
        end)
    ...
    local function queueMob(model)
        if not (states.mobSpawnOnMe or states.mobHardHide) then return end
        if not (model and model.Parent and model:IsA("Model")) then return end
        -- Root movement is intentionally immediate: this is cheap compared to visual/particle work
        -- and prevents the wave from sitting at the SPAWNS area.
        if states.mobSpawnOnMe then
            moveMobToMe(model)
            task.delay(0.05, function()
                if GEN == _G.MeleeRNG_Gen and states.mobSpawnOnMe then moveMobToMe(model) end
            end)
            task.delay(0.14, function()
                if GEN == _G.MeleeRNG_Gen and states.mobSpawnOnMe then moveMobToMe(model) end
            end)
        end
        forceOwnWeaponHideSoon()
        if mobQueued[model] then return end
        mobQueued[model] = true
        mobQueue[#mobQueue + 1] = model
    end

    local function processQueuedMob(model)
        if not (model and model.Parent and model:IsA("Model")) then return false end
        if states.mobSpawnOnMe then moveMobToMe(model) end
        if states.mobHardHide then applyMobHardHide(model, true) end
        return true
    end

    local function queueExistingMobs(limit)
    ...
    local function processMobQueue(maxCount)
        local done = 0
        maxCount = math.max(1, math.floor(tonumber(maxCount) or MOB_QUEUE_BATCH))
        while done < maxCount and #mobQueue > 0 do
            local m = table.remove(mobQueue, 1)
            mobQueued[m] = nil
            if processQueuedMob(m) then
                done = done + 1
                mobProcessed = mobProcessed + 1
            end
        end
        return done
    end

    local function restoreMobVisuals()
        for part, parent in pairs(mobVisualParent) do
            pcall(function()
                if part and part.Parent and parent and parent.Parent then part.Parent = parent end
            end)
            mobVisualParent[part] = nil
        end
        for part, old in pairs(mobHidden) do
            pcall(function() if part and part.Parent then part.LocalTransparencyModifier = old end end)
            mobHidden[part] = nil
        end
        for fx, old in pairs(mobFx) do
            pcall(function() if fx and fx.Parent then fx.Enabled = old end end)
            mobFx[fx] = nil
        end
    end

    local function cleanupMobCaches()
        local now = os.clock()
        if now - mobLastCleanup < 4 then return end
        mobLastCleanup = now
        ...
        for m in pairs(mobQueued) do if not (m and m.Parent) then mobQueued[m] = nil end end
        for p, parent in pairs(mobVisualParent) do
            if not (p and p.Parent) then
                mobVisualParent[p] = nil
            elseif not (parent and parent.Parent) then
                -- The mob/model that owned this visual is gone; delete the stashed part so hidden visuals do not build up.
    ...
    MiscTab:Toggle("Mobs spawn/move on me", "Moves spawned mobs to your character immediately; visual hiding is handled separately.", states.mobSpawnOnMe, function(on)
        states.mobSpawnOnMe = on == true
        saveSettings()
        if states.mobSpawnOnMe then queueExistingMobs(nil) end
        safeSet(mobStatus, states.mobSpawnOnMe and "Mob move ON · instant roots" or "Mob move OFF")
    end)
    MiscTab:Toggle("Hard-hide mob visuals", "Defers only heavy mob visuals/effects so roots can still move instantly.", states.mobHardHide, function(on)
        ...
        if not states.mobHardHide then
            ...
            queueExistingMobs(nil)
            safeSet(mobStatus, "Mob hard-hide ON · deferred visuals")
        end
    end)
    MiscTab:Button("🧲", "Move all mobs to me now", "Moves current workspace.Mobs models to you immediately, then queues visual hiding.", C.green, function()
        local n = queueExistingMobs(nil)
        safeSet(mobStatus, "Moved/queued " .. tostring(n) .. " mobs")
    end)

    addThread(task.spawn(function()
        local lastFolder = nil
        local nextBackupSweep = 0
        while true do
            if GEN ~= _G.MeleeRNG_Gen then break end
            local f = workspace:FindFirstChild("Mobs")
            if f and f ~= lastFolder then
                ...
                addConn(f.ChildAdded:Connect(function(ch)
                    ...
                    if ch:IsA("Model") then
                        -- New mob models often get children over the next few frames; move now and re-apply shortly.
                        task.defer(function() queueMob(ch) end)
                        task.delay(0.04, function() if GEN == _G.MeleeRNG_Gen then queueMob(ch) end end)
                        task.delay(0.16, function() if GEN == _G.MeleeRNG_Gen then queueMob(ch) end end)
                    end
                end))
            end

            if states.mobSpawnOnMe or states.mobHardHide then
                if os.clock() >= nextBackupSweep then
                    queueExistingMobs(20)
                    nextBackupSweep = os.clock() + MOB_BACKUP_SWEEP_SEC
                end
                local done = processMobQueue(MOB_QUEUE_BATCH)
                if done > 0 and os.clock() >= mobNextStatus then
                    mobNextStatus = os.clock() + 0.75
                    safeSet(mobStatus, "Mobs stashed: " .. tostring(mobProcessed) .. " · queue=" .. tostring(#mobQueue))
                end
                cleanupMobCaches()
                task.wait((#mobQueue > 0) and 0.025 or 0.45)
            else
                task.wait(1.5)
            end
        end
    ...
    MiscTab:Input("Leave/restart at Room_N", "Optional. 0/blank disables exact room targeting; enter a number only when you want auto-leave.", "0", tostring(states.timeTrialLeaveAtRoom), function(txt)
        local raw = tostring(txt or "")
        local parsed = tonumber(raw:match("%d+") or raw) or 0
        states.timeTrialLeaveAtRoom = math.clamp(math.floor(parsed), 0, 1000000)
        saveSettings()
        safeSet(ttLabel, states.timeTrialLeaveAtRoom > 0 and ("Auto leave target: Room_" .. tostring(states.timeTrialLeaveAtRoom)) or "Auto leave target disabled")
    end)
    MiscTab:Toggle("Auto leave + restart at room", "Uses MainGUI.GeneralUI.LeaveTimeTrialBtn when your target Room_N appears, then starts another run.", states.timeTrialAutoLeaveRestart, function(on)
        ...
        saveSettings()
        safeSet(ttLabel, states.timeTrialAutoLeaveRestart and (states.timeTrialLeaveAtRoom > 0 and ("Auto leave/restart at Room_" .. tostring(states.timeTrialLeaveAtRoom)) or "Auto leave/restart ON · no room target") or "Auto leave/restart OFF")
    end)
    ...
    addThread(task.spawn(function()
        ...
        while true do
            ...
            if states.timeTrialAutoLeaveRestart then
                local target = math.clamp(math.floor(tonumber(states.timeTrialLeaveAtRoom) or 0), 0, 1000000)
                local room = nil
                if target > 0 then
                    room = findRoom(target)
                    local newest, newestN = latestRoom()
                    if not room and newestN and newestN >= target then
                        room = newest
                    end
                end
                if target > 0 and room and room ~= lastLeaveModel and os.clock() >= nextLeaveAllowed and meleeFindTimeTrialLeaveButton() then
                    lastLeaveModel = room
...
local _perfWeaponParent = {} -- [BasePart] = original parent while visual weapon parts are stashed out of Workspace

local function perfHiddenWeaponFolder()
    local pg = LP and LP:FindFirstChildOfClass("PlayerGui")
    if not pg then return nil end
    local f = pg:FindFirstChild("MeleeRNG_HiddenWeaponVisuals")
    if not f then
        f = Instance.new("Folder")
        f.Name = "MeleeRNG_HiddenWeaponVisuals"
        f.Parent = pg
    end
    return f
end
...
local function perfCleanupStalePartKeys()
    ...
    for p, parent in pairs(_perfWeaponParent) do
        if not (p and p.Parent) then
            _perfWeaponParent[p] = nil
        elseif not (parent and parent.Parent) then
            -- Original weapon model was rebuilt/destroyed; remove the hidden visual copy so it cannot pile up later.
            pcall(function() p:Destroy() end)
            _perfWeaponParent[p] = nil
            _perfPartLTM[p] = nil
        end
...
local function perfApplyWeaponVisualDesc(d, hide)
    ...
    if d:IsA("BasePart") then
        ...
        if hide then
            ...
            d.LocalTransparencyModifier = 1
            pcall(function() d.CastShadow = false end)
            -- LTM hides rendering, but the part still exists in Workspace. Stashing visuals removes their
            -- render/physics cost while keeping Hitbox parts in the weapon model for combat.
            if d.Parent and not _perfWeaponParent[d] then
                local stash = perfHiddenWeaponFolder()
                if stash then
                    _perfWeaponParent[d] = d.Parent
                    pcall(function() d.Parent = stash end)
                end
            end
        else
            local originalParent = _perfWeaponParent[d]
            if originalParent and originalParent.Parent and d.Parent then
                pcall(function() d.Parent = originalParent end)
            end
            _perfWeaponParent[d] = nil
...
local function perfParentBelongsToFolder(parent, folder)
    if not (parent and folder) then return false end
    if parent == folder then return true end
    return parent:IsDescendantOf(folder)
end

local function perfRestoreWeaponStashForFolder(folder)
    for part, parent in pairs(_perfWeaponParent) do
        if (not folder) or perfParentBelongsToFolder(parent, folder) then
            pcall(function()
                if part and part.Parent and parent and parent.Parent then part.Parent = parent end
            end)
            _perfWeaponParent[part] = nil
            local prev = _perfPartLTM[part]
            if prev ~= nil then
                pcall(function() if part and part.Parent then part.LocalTransparencyModifier = prev end end)
...
local function perfSweepWeaponFolder(folder, hide)
    if not folder then return end
    if not hide then
        -- Stashed weapon visuals are no longer descendants of the weapon folder, so restore them first.
        perfRestoreWeaponStashForFolder(folder)
    end
...
local _perfOwnWeaponChildConn = nil

local function perfHideWeaponObjectFast(obj)
    if not obj then return end
    if obj:IsA("BasePart") or obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam") or obj:IsA("PointLight") or obj:IsA("SpotLight") or obj:IsA("SurfaceLight") then
        perfApplyWeaponVisualDesc(obj, true)
        return
    end
...
local function perfHideWeaponObjectRepeated(obj)
    if not states.perfHideOwnWeapons then return end
    perfHideWeaponObjectFast(obj)
    task.delay(0.03, function()
        if GEN == _G.MeleeRNG_Gen and states.perfHideOwnWeapons and obj and obj.Parent then
            perfHideWeaponObjectFast(obj)
        end
    end)
    task.delay(0.14, function()
...
function _G.meleeForceHideOwnWeaponVisualsNow()
    if not states.perfHideOwnWeapons then return false end
    local folder = workspace:FindFirstChild(LP.Name)
    if not folder then return false end
    perfSweepWeaponFolder(folder, true)
    return true
end
...
local function perfUnwatchOwnWeaponFolder()
    ...
    if _perfOwnWeaponChildConn then
        pcall(function() _perfOwnWeaponChildConn:Disconnect() end)
        _perfOwnWeaponChildConn = nil
    end
...
local function perfWatchOwnWeaponFolderIfNeeded()
    ...
    if folder then
        _perfOwnWeaponChildConn = folder.ChildAdded:Connect(function(ch)
            if GEN ~= _G.MeleeRNG_Gen then return end
            if states.perfHideOwnWeapons then
                task.defer(function()
                    if GEN ~= _G.MeleeRNG_Gen or not (ch and ch.Parent) then return end
                    perfHideWeaponObjectRepeated(ch)
                end)
            end
            _hitboxCacheDirty = true
        end)
        addConn(_perfOwnWeaponChildConn)
        ...
        if states.perfHideOwnWeapons then
            task.defer(function()
                if GEN == _G.MeleeRNG_Gen and states.perfHideOwnWeapons then
                    perfSweepWeaponFolder(folder, true)
                end
            end)
        end
...
local function perfRestoreAllWeaponFolders()
    ...
    -- Catch any stashed visuals whose original folder is no longer in workspace.
    perfRestoreWeaponStashForFolder(nil)
