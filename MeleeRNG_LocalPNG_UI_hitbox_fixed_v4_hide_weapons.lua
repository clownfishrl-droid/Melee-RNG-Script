...
local function meleeGuiMountHitboxPulseSection(MoveTab, SLbl, C)
    ...
    local HITBOX_BATCH_MAX    = 16    -- hard cap: more swords no longer means every sword is resized in one frame
    local HITBOX_HOLD_FRAMES  = 2     -- keep active batch BIG long enough for physics/Touched to catch it
    local HITBOX_MOB_SCAN_SEC = 0.12  -- throttle workspace.Mobs scans
    local HITBOX_TOUCH_ASSIST_BUDGET = 768 -- max local touch nudges per pulse; direct-signal assist is cheap and needs enough hits for true one-shot
    local HITBOX_TOUCHES_PER_TARGET = 24   -- each mob gets up to this many weapon-hit nudges per pulse before rolling onward
...
local function perfSetVisualPartHidden(part, hide)
    ...
    if hide then
        if _perfPartLTM[part] == nil then
            _perfPartLTM[part] = part.LocalTransparencyModifier
        end
    ...
    for _, m in ipairs(folder:GetChildren()) do
        if m:IsA("Model") then
            for _, d in ipairs(m:GetDescendants()) do
                if d:IsA("BasePart") then
                    perfSetVisualPartHidden(d, hide)
                end
            end
        end
    end
...
addThread(task.spawn(function()
    while true do
        ...
        -- Do not sweep weapon folders every Heartbeat. That was the main Hide Weapons lag source.
        -- Rescan a few times per second so newly equipped/spawned weapon visuals still hide.
...
;(function()
    ...
    MiscTab:Label("Client-only FPS helpers. Hide Own Weapons now skips combat Hitbox parts and uses throttled sweeps.")
    local perfUi = {}
    ...
    perfUi.own = MiscTab:Toggle("Hide Own Weapons", "Attack-safe: hides visual weapon parts under your workspace player folder, skips parts named Hitbox, and sweeps slower to reduce lag — saved", states.perfHideOwnWeapons, function(on)
        states.perfHideOwnWeapons = on == true
        saveSettings()
        pcall(function() perfSweepWeaponFolder(workspace:FindFirstChild(LP.Name), on == true) end)
        _perfWeaponAcc = 0
        SLbl.Text = on and "Perf: your weapon visuals hidden (Hitboxes kept active)" or "Perf: your weapons shown"
    end)
