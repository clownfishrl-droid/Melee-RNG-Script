...
function meleeMountSafeDungeonMobTimeTrialAddons(MiscTab, SLbl, C)
    ...
    -- Mob anti-lag: move roots immediately, but defer heavy visual hiding.
    -- This keeps mobs arriving on you instantly without doing every visual-descendant edit in one frame.
    local mobHidden = {}
    local mobFx = {}
    local mobDecalTrans = {}
    local mobDescConns = {}
    ...
    -- New wave-safe mob hider:
    -- move roots instantly, but make visuals/physics cheap immediately without reparenting anything.
    local MOB_QUEUE_BATCH = 18      -- backup only; mob movement is immediate
    local MOB_DESC_BUDGET = 160     -- higher budget = fewer task yields while suppressing a wave
    ...
    local function mobDisableVisualDesc(d, root, hide)
        if not d then return end
        if d:IsA("BasePart") then
            -- Keep the root touchable/queryable enough for existing targeting/hitbox logic,
            -- but remove collision/render cost from the rest of the mob.
            if d == root then
                pcall(function() d.CastShadow = false end)
                pcall(function() d.CanCollide = false end)
                pcall(function() d.Massless = true end)
                -- Do NOT force root CanTouch off; client hitbox/touch logic may need it.
                return
            end
            if hide then
        ...
        elseif d:IsA("Decal") or d:IsA("Texture") then
            if hide then
                if mobDecalTrans[d] == nil then mobDecalTrans[d] = d.Transparency end
                d.Transparency = 1
            else
                local old = mobDecalTrans[d]
                if old ~= nil then
                    d.Transparency = old
                    mobDecalTrans[d] = nil
                end
    ...
    local function unwatchMobModel(model)
        local c = mobDescConns[model]
        if c then
            pcall(function() c:Disconnect() end)
            mobDescConns[model] = nil
        end
    end

    local function watchMobModel(model)
        if not (model and model.Parent) or mobDescConns[model] then return end
        local conn = model.DescendantAdded:Connect(function(d)
            if GEN ~= _G.MeleeRNG_Gen then return end
            if states.mobHardHide then
                -- New mob descendants often stream in after the Model appears; suppress them immediately.
                local root = mobRoot(model)
                mobDisableVisualDesc(d, root, true)
            end
        end)
        mobDescConns[model] = conn
        addConn(conn)
    end
    ...
    local function applyMobHardHide(model, hide)
        ...
        local root = mobRoot(model)
        local budget = 0
        if hide then watchMobModel(model) else unwatchMobModel(model) end
        for _, d in ipairs(model:GetDescendants()) do
            mobDisableVisualDesc(d, root, hide)
            budget = budget + 1
    ...
    local function queueMob(model)
        ...
        -- Root movement stays instant. The previous visual queue could build up during waves;
        -- this version suppresses mob visual/physics cost immediately and watches late-added parts.
        ...
        if states.mobHardHide then
            applyMobHardHide(model, true)
            task.delay(0.22, function()
                if GEN == _G.MeleeRNG_Gen and states.mobHardHide and model and model.Parent then
                    applyMobHardHide(model, true)
                end
            end)
        end
    ...
    local function restoreMobVisuals()
        ...
        for dec, old in pairs(mobDecalTrans) do
            pcall(function() if dec and dec.Parent then dec.Transparency = old end end)
            mobDecalTrans[dec] = nil
        end
        for m, c in pairs(mobDescConns) do
            pcall(function() c:Disconnect() end)
            mobDescConns[m] = nil
        end
    end
    ...
    local function cleanupMobCaches()
        local now = os.clock()
        if now - mobLastCleanup < 5 then return end
        mobLastCleanup = now
        ...
        for dec in pairs(mobDecalTrans) do if not (dec and dec.Parent) then mobDecalTrans[dec] = nil end end
        for m, c in pairs(mobDescConns) do
    ...
    MiscTab:Toggle("Hard-hide mob visuals", "Immediately disables mob visuals/collisions but keeps roots active for targeting.", states.mobHardHide, function(on)
        ...
        if not states.mobHardHide then
            ...
            safeSet(mobStatus, "Mob hard-hide ON · instant visual suppression")
        end
    end)
    MiscTab:Button("🧲", "Move all mobs to me now", "Moves current workspace.Mobs models to you immediately and suppresses mob visuals.", C.green, function()
    ...
    addThread(task.spawn(function()
        ...
        while true do
            ...
            if states.mobSpawnOnMe or states.mobHardHide then
                ...
                if done > 0 then
                    safeSet(mobStatus, "Mobs processed: " .. tostring(mobProcessed) .. " · queue=" .. tostring(#mobQueue))
                end
                cleanupMobCaches()
                task.wait((#mobQueue > 0) and 0.06 or 0.35)
            else
                task.wait(1.5)
            end
        end
