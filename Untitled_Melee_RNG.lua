...
local states = {
    ...
    upgradeSequentialMode = true, -- Target Chain: buy first ON upgrade to its target level, then move to the next. false = cheapest-first legacy mode.
...
local upgradeLevelCap = {} -- [upgradeName] = target/stop level; Target Chain uses it as the required level before moving to the next row. nil = no target/no cap
...
states.upgradeSequentialMode = states.upgradeSequentialMode ~= false
...
local function meleeGuiMountMiscAscendSection(MiscTab, SLbl, C)
    ...
    MiscTab:Label(
        "Step 2 — Evaluate upgrade targets: in Target Chain mode, Upgrades-tab rows that are ON with a target level are completed top-to-bottom. Rows with no target are skipped. Cheapest First mode uses the old queue behavior.")
    MiscTab:Label(
    ...
    MiscTab:Label(
        "Raid line “X/Y below target”: X = ON rows still under their target; Y = how many ON rows have targets. If Y = 0 you see “targets —”; Target Chain skips rows with no target.")
    MiscTab:Label(
...
-- --------------------------------------------------------------------------
local Win  = Lib:Window({ title = "⚔️  Melee RNG  v1.1 Target Upgrades" })
...
_G.MeleeRNG__G.MeleeRNG_UPGRADE_DESCRIPTIONS = {
...
--- Target Chain behavior: use UI order, buy only the first ON row that is below its target.
...
--- Main auto-buy queue. Target Chain = one active target at a time; Cheapest First = old cheapest queue.
local function upgBuildSortedCandidates()
    if states.upgradeSequentialMode then
        return upgBuildSequentialCandidates()
    end
    return upgBuildCheapestCandidates()
end
...
--- Status summary: Target Chain shows the level plan; Cheapest First shows next SP prices.
...
local function upgModeName()
    return states.upgradeSequentialMode and "Target Chain" or "Cheapest First"
end
...
UpgTab:Label("Mode: Target Chain buys the first ON upgrade to its target level, then moves down the list. Cheapest First buys the lowest-cost ON upgrade.")
...
UpgTab:Button("🎯", "Switch Upgrade Mode", "Target Chain = finish one target level before the next. Cheapest First = older lowest-cost behavior.", C.purple, function()
    states.upgradeSequentialMode = not states.upgradeSequentialMode
    setAutoUpgrade(states.autoUpgradeOn)
    saveSettings()
    SLbl.Text = "Upgrade mode: " .. upgModeName()
end)

UpgTab:Label("Target Chain mode: rows are read top to bottom. Turn a row ON, enter a target level, and Auto Upgrade buys only that row until it reaches the target. Then it moves to the next ON row with a target.")
UpgTab:Label("Cheapest First mode: older behavior. It buys whichever ON row has the lowest next SP cost. A target level still acts as a stop; an empty target means no stop.")
UpgTab:Section("Upgrade Targets (SP)")
UpgTab:Label("Example: Damage Multiplier ON + Target 150 means buy Damage Multiplier to L150 first, then continue to the next ON target below it.")
...
for _, name in ipairs(UPGRADE_ORDER) do
    ...
    local n2    = name
    upgToggles[name] = UpgTab:Toggle(name, ((_G.MeleeRNG_UPGRADE_DESCRIPTIONS[n2] or "SP upgrade.") .. " Target Chain: ON + target level = buy this before moving to the next target."), isSel, function(on)
    ...
    UpgTab:Input(
        "Target level",
        n2 .. " — Target Chain stops here before the next row. Empty = skipped in Target Chain / no cap in Cheapest First. Saved.",
        "target level",
        capDefault,
        function(txt)
...
local function upgFormatLiveRowText(name)
    ...
    elseif not capSet and states.upgradeSequentialMode then
        tail = " · no target set (skipped in Target Chain)"
    elseif not capSet then
        tail = " · no cap"
    else
        tail = ""
    end
    return name .. " — " .. (_G.MeleeRNG_UPGRADE_DESCRIPTIONS[name] or "SP upgrade") .. " | " .. mid .. tail
end
    ...
    while true do
        ...
        if states.upgradeSequentialMode then
            ...
            if row then
                ...
                upgInfoLbl.Set(string.format("SP: %s | Target Chain complete / no target below level | %s",
...
-- ── Auto Upgrade Loop ─────────────────────────────────────────────────────
-- Burst-buy: Target Chain buys one active target at a time; Cheapest First buys the lowest next SP price.
        ...
        if not boughtAny then
            ...
            if states.upgradeSequentialMode then
                ...
                if row then
                    ...
                    upgStatusLbl.Set("✓ Target Chain complete — no ON target is below its level")
                end
            else
                local nextName, nextPrice = "none", math.huge
                if #candidates > 0 then
...
addThread(task.spawn(function()
    ...
    --- True when no raid SP spend is needed: Target Chain has no unfinished target, or Cheapest First has no remaining auto-buy candidates.
    -- Target Chain intentionally skips ON rows with no target level so it can move through a clean top-to-bottom plan.
