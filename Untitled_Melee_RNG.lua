...
-- First-person Roblox camera scripts can force LockCenter every frame; when ON, keep overriding to Default.
local mouseUnlockToggle = nil
local mouseUnlockStatusLbl = nil
local function mouseUnlockStatusText()
    return states.mouseUnlockN and "Mouse unlock: ON — press N to let the game control the mouse again" or "Mouse unlock: OFF — press N to unlock mouse in first person"
end
...
local function applyMouseUnlockN()
    if states.mouseUnlockN ~= true then return end
    pcall(function()
...
-- --------------------------------------------------------------------------
local Win  = Lib:Window({ title = "⚔️  Melee RNG  v1.1.2 Target Upgrades + Mouse N" })
...
MiscTab:Button("🛑", "Stop All Loops", "Disconnects all connections and cancels all threads", C.red, function()
    ...
    states.mouseUnlockN = false
    pcall(mouseUnlockRefreshUi)
    pcall(function()
...
MiscTab:Label("Press N to toggle mouse unlock. When ON, first-person mouse lock is forced off every frame so your cursor stays free.")
mouseUnlockStatusLbl = MiscTab:Label(mouseUnlockStatusText())
mouseUnlockToggle = MiscTab:Toggle("🖱️ Unlock Mouse with N", "Hotkey: N. ON keeps MouseBehavior = Default and MouseIconEnabled = true; OFF lets the game control first-person mouse lock again.", states.mouseUnlockN, function(on)
