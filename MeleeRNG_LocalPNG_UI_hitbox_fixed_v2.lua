...
local function meleeGuiMountHitboxPulseSection(MoveTab, SLbl, C)
    ...
    local HITBOX_TOUCH_ASSIST_BUDGET = 256 -- max local touch nudges per pulse; high enough to one-shot without resizing every sword
    ...
    pcall(function()
        if type(firetouchinterest) == "function" then
            _firetouchinterest = firetouchinterest
        end
    end)
    ...
    MoveTab:Label("Reliability: BIG is held for 2 frames and local Touched listeners are nudged when available, so hits are less likely to miss under load.")
    ...
    local function fireLocalTouched(hitbox, otherPart)
        ...
        -- Executor/local environments often expose firetouchinterest. Use it if present.
        if type(_firetouchinterest) == "function" then
            local ok0 = pcall(_firetouchinterest, hitbox, otherPart, 0)
            local ok1 = pcall(_firetouchinterest, hitbox, otherPart, 1)
            fired = fired or ok0 or ok1
        end

        -- If the local weapon script uses Hitbox.Touched, nudge that signal too.
        local fs = rawget(_G, "firesignal")
        if type(fs) == "function" then
            local ok = pcall(fs, hitbox.Touched, otherPart)
            fired = fired or ok
        end

        -- Last fallback: call visible local Touched connections directly.
        if type(_getconnections) == "function" then
            local ok, conns = pcall(_getconnections, hitbox.Touched)
            if ok and type(conns) == "table" then
                for _, c in ipairs(conns) do
    ...
    local function pulseTouchAssist(activeHitboxes)
        if type(_firetouchinterest) ~= "function" and type(rawget(_G, "firesignal")) ~= "function" and type(_getconnections) ~= "function" then
            return 0
        end
        ...
        local firedCount = 0
        local checked = 0
        local maxChecks = math.max(HITBOX_TOUCH_ASSIST_BUDGET * 3, hitboxCount * math.min(targetCount, 3))

        -- First nudge the actually BIG batch. This gives the physics path and the
        -- direct local Touched path the same targets right away.
                ...
                if hb and hb.Parent then
                    ...
                    for ti = 1, targetCount do
                        ...
                        if target and target.Parent and (target.Position - hbPos).Magnitude <= maxDist then
                            ...
                            if firedCount >= HITBOX_TOUCH_ASSIST_BUDGET then
                                return firedCount
                            end
                        end
                    end
        ...
        -- Then spend the remaining budget across ALL weapon hitboxes in a rolling
        -- cursor. That preserves the "all swords contribute" behavior without
        -- resizing every sword in the same Heartbeat.
        ...
        while firedCount < HITBOX_TOUCH_ASSIST_BUDGET and checked < maxChecks do
            checked = checked + 1
        ...
        if _pulseActive then
            ...
            _pulseConn = RunService.Heartbeat:Connect(function()
                ...
                if now - _pulseLastStatusAt >= 1.0 then
                    ...
                    SLbl.Text = string.format(
                        "⚡ Pulse ON | %.0f studs | %d hitboxes | batch %d | duty 1/%d | local touches %d",
                        hitboxSize, total, batch, duty, touched
                    )
                end
            end)
            ...
            rebuildHitboxCache()
            SLbl.Text = string.format(
                "⚡ Pulse ON | %.0f studs | %d hitboxes | batch %d | duty 1/%d",
                hitboxSize, #_hitboxCache, currentHitboxBatchSize(#_hitboxCache), currentHitboxDuty()
            )
        else
            if _pulseConn then _pulseConn:Disconnect(); _pulseConn = nil end
            restoreHitboxes()
