...
local states = {
    ...
    uiVideoBackground = true, -- Neon UI revamp: MP4/gradient background behind NexusLib
    autoReExec     = false,
    hitboxPulse    = false,
...
-- ── UI Revamp + MP4 background ─────────────────────────────────
-- Put this file next to the script in the executor workspace for local-video backgrounds.
-- If local videos are blocked/missing, the theme falls back to a neon gradient and still works.
local UI_BG_VIDEO_FILE = "MeleeRNG_UI_Background.mp4"
-- GitHub raw MP4 fallback. Keep the MP4 uploaded next to this Lua in the repo, or replace this with your own raw URL.
...
local UI_BG_ASSET_ID = "" -- Optional: set to a Roblox video asset id if you upload the MP4 to Roblox.
local UI_BG_LAST_STATUS = "not checked"
local UI_REVAMP_GUI_ATTR = "MeleeRNG_NeonRevamp"
local _uiRevampRoot = nil
local _uiRevampConn = nil

local function uiTrySet(inst, prop, value)
    pcall(function() inst[prop] = value end)
end

local function uiBgFileExists()
    if type(isfile) ~= "function" then return false end
    local ok, exists = pcall(isfile, UI_BG_VIDEO_FILE)
    return ok and exists == true
end

local function uiLooksLikeMp4(data)
    if type(data) ~= "string" or #data < 20000 then return false end
    -- Normal MP4 files contain the ftyp box near the start. This also rejects GitHub 404/html pages.
    return data:sub(5, 8) == "ftyp" or data:sub(1, 96):find("ftyp", 1, true) ~= nil
end

local function uiEnsureBgVideoDownloaded(forceRefresh)
    if not forceRefresh and uiBgFileExists() then
        UI_BG_LAST_STATUS = "local file ready"
        return true
    end
    ...
    if not ok or type(data) ~= "string" then
        UI_BG_LAST_STATUS = "download failed"
        return false
    end
    if not uiLooksLikeMp4(data) then
    ...
    if not wrote then
        UI_BG_LAST_STATUS = "write failed: " .. tostring(err)
        return false
    end

    UI_BG_LAST_STATUS = "downloaded from GitHub"
    return uiBgFileExists()
end

local function uiGetCustomAsset(path)
    local fn = nil
...
local function uiGetBgVideoSource()
    if type(UI_BG_ASSET_ID) == "string" and UI_BG_ASSET_ID ~= "" then
        UI_BG_LAST_STATUS = "using Roblox asset id"
        return "rbxassetid://" .. UI_BG_ASSET_ID
    end

    -- If the MP4 is not already in the executor workspace, pull it from GitHub once.
    uiEnsureBgVideoDownloaded(false)

    if uiBgFileExists() then
        local asset = uiGetCustomAsset(UI_BG_VIDEO_FILE)
        if asset then
            UI_BG_LAST_STATUS = UI_BG_LAST_STATUS .. " + loaded"
            return asset
        end
        UI_BG_LAST_STATUS = "getcustomasset/getsynasset missing or blocked"
    end
    return nil
end

local function uiFindMeleeRoot()
    local pg = LP and LP:FindFirstChildOfClass("PlayerGui")
    if not pg then return nil end
    if _uiRevampRoot and _uiRevampRoot.Parent == pg then return _uiRevampRoot end
        ...
        if sg:IsA("ScreenGui") and sg.Name ~= MOUSE_UNLOCK_GUI_NAME then
            ...
            for _, d in ipairs(sg:GetDescendants()) do
                if d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox") then
                    ...
                    if t:find("Auto Farm", 1, true) then score = score + 8 end
...
local function uiEnsureCorner(obj, px)
...
local function uiEnsureStroke(obj, color, transparency, thickness)
    local st = obj:FindFirstChild("MeleeRNG_NeonStroke")
    if not st then
        st = Instance.new("UIStroke")
        st.Name = "MeleeRNG_NeonStroke"
...
local function uiIsRevampBackground(obj)
    return obj and (obj.Name == "MeleeRNG_VideoBackground" or obj.Name == "MeleeRNG_BackgroundDim" or obj.Name == "MeleeRNG_BackgroundGlow")
end

local function uiStyleObject(obj)
    if not obj or uiIsRevampBackground(obj) then return end
    if obj:GetAttribute(UI_REVAMP_GUI_ATTR) then return end
    obj:SetAttribute(UI_REVAMP_GUI_ATTR, true)

    if obj:IsA("Frame") or obj:IsA("ScrollingFrame") then
        local sx = obj.AbsoluteSize.X
        local sy = obj.AbsoluteSize.Y
        if sx > 12 and sy > 12 then
            uiEnsureCorner(obj, 14)
            uiEnsureStroke(obj, Color3.fromRGB(70, 210, 255), 0.74, 1)
        end
        if obj:IsA("ScrollingFrame") then
            uiTrySet(obj, "ScrollBarImageColor3", Color3.fromRGB(0, 225, 255))
            uiTrySet(obj, "ScrollBarThickness", 4)
        end
        if obj.BackgroundTransparency < 0.98 then
            obj.BackgroundColor3 = Color3.fromRGB(8, 10, 20)
            obj.BackgroundTransparency = math.max(obj.BackgroundTransparency, 0.16)
        end
    elseif obj:IsA("TextButton") then
        uiEnsureCorner(obj, 12)
        uiEnsureStroke(obj, Color3.fromRGB(105, 92, 255), 0.48, 1)
        obj.AutoButtonColor = true
        if obj.BackgroundTransparency < 0.98 then
            obj.BackgroundColor3 = Color3.fromRGB(16, 20, 42)
            obj.BackgroundTransparency = math.max(obj.BackgroundTransparency, 0.08)
        end
        obj.Font = Enum.Font.GothamSemibold
        obj.TextColor3 = Color3.fromRGB(235, 245, 255)
        obj.TextSize = math.max(obj.TextSize, 12)
    elseif obj:IsA("TextBox") then
        uiEnsureCorner(obj, 12)
        uiEnsureStroke(obj, Color3.fromRGB(255, 79, 216), 0.55, 1)
        if obj.BackgroundTransparency < 0.98 then
            obj.BackgroundColor3 = Color3.fromRGB(13, 16, 32)
            obj.BackgroundTransparency = math.max(obj.BackgroundTransparency, 0.06)
        end
        obj.ClearTextOnFocus = false
        obj.Font = Enum.Font.GothamMedium
        obj.TextColor3 = Color3.fromRGB(245, 245, 255)
        obj.PlaceholderColor3 = Color3.fromRGB(115, 135, 170)
    elseif obj:IsA("TextLabel") then
        obj.BackgroundTransparency = math.max(obj.BackgroundTransparency, 0.2)
        obj.Font = (obj.TextSize >= 14) and Enum.Font.GothamBold or Enum.Font.GothamMedium
        obj.TextColor3 = Color3.fromRGB(228, 238, 255)
        if tostring(obj.Text or ""):find("Status", 1, true) or tostring(obj.Text or ""):find("—", 1, true) then
            obj.TextColor3 = Color3.fromRGB(151, 190, 235)
        end
...
local function uiApplyBackground(root, enabled)
    if not root then return end
    local oldVideo = root:FindFirstChild("MeleeRNG_VideoBackground")
    local oldDim = root:FindFirstChild("MeleeRNG_BackgroundDim")
    local oldGlow = root:FindFirstChild("MeleeRNG_BackgroundGlow")
    if not enabled then
        if oldVideo then oldVideo:Destroy() end
        if oldDim then oldDim:Destroy() end
        if oldGlow then oldGlow:Destroy() end
        return
    end

    local video = oldVideo
    if not video then
        video = Instance.new("VideoFrame")
        video.Name = "MeleeRNG_VideoBackground"
        video.Size = UDim2.new(1, 0, 1, 0)
        video.Position = UDim2.new(0, 0, 0, 0)
        video.BackgroundColor3 = Color3.fromRGB(4, 5, 12)
        video.BorderSizePixel = 0
        video.ZIndex = 0
        video.Parent = root
    end
    video.Visible = true
    uiTrySet(video, "ScaleType", Enum.ScaleType.Crop) -- fit/fill without stretching; crops edges if aspect differs
    uiTrySet(video, "Looped", true)
    uiTrySet(video, "Volume", 0)
    uiTrySet(video, "VideoTransparency", 0.12)

    local src = uiGetBgVideoSource()
    if src then
        uiTrySet(video, "Video", src)
        pcall(function() video:Play() end)
    else
        -- Gradient fallback if the MP4 is not next to the script or the executor blocks local videos.
        video.Visible = false
    end
    ...
    if not glow then
        ...
        glow.Name = "MeleeRNG_BackgroundGlow"
        glow.Size = UDim2.new(1, 0, 1, 0)
        glow.Position = UDim2.new(0, 0, 0, 0)
        glow.BorderSizePixel = 0
        glow.BackgroundColor3 = Color3.fromRGB(10, 12, 25)
        glow.BackgroundTransparency = src and 0.45 or 0
        glow.ZIndex = 0
        local grad = Instance.new("UIGradient")
        grad.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(9, 13, 32)),
            ColorSequenceKeypoint.new(0.42, Color3.fromRGB(18, 10, 34)),
            ColorSequenceKeypoint.new(1, Color3.fromRGB(3, 28, 42)),
        })
        grad.Rotation = 35
        grad.Parent = glow
        glow.Parent = root
        addThread(task.spawn(function()
            while glow.Parent and GEN == _G.MeleeRNG_Gen do
                for rot = 35, 395, 2 do
                    if not glow.Parent or GEN ~= _G.MeleeRNG_Gen then return end
                    grad.Rotation = rot
                    task.wait(0.08)
                end
            end
    ...
    if not dim then
        ...
        dim.Name = "MeleeRNG_BackgroundDim"
...
local function meleeApplyUiRevamp(enabled)
    local root = uiFindMeleeRoot()
    if not root then return false end
    root.ResetOnSpawn = false
    root.IgnoreGuiInset = true
    uiApplyBackground(root, enabled ~= false)

    for _, d in ipairs(root:GetDescendants()) do
        uiStyleObject(d)
    end
    if _uiRevampConn then pcall(function() _uiRevampConn:Disconnect() end) end
    _uiRevampConn = root.DescendantAdded:Connect(function(d)
        task.defer(function()
            if GEN ~= _G.MeleeRNG_Gen then return end
            uiStyleObject(d)
        end)
...
-- --------------------------------------------------------------------------
local Win  = Lib:Window({ title = "⚔️  Melee RNG · Neon Suite" })
local SLbl = Win._status

-- Shared palette / status cards (used by every tab)
local C = {
    bg=Color3.fromRGB(5,7,16), card=Color3.fromRGB(11,15,31),
    border=Color3.fromRGB(46,62,102), accent=Color3.fromRGB(0,225,255),
    accentD=Color3.fromRGB(0,118,180), text=Color3.fromRGB(235,245,255),
    sub=Color3.fromRGB(120,150,195), green=Color3.fromRGB(0,235,150),
    purple=Color3.fromRGB(175,95,255), red=Color3.fromRGB(255,82,104),
    orange=Color3.fromRGB(255,165,65), gold=Color3.fromRGB(255,224,90),
    pink=Color3.fromRGB(255,79,216), glass=Color3.fromRGB(12,18,38),
}
...
-- Tabs: Movement → Visuals → Zones → Upgrade Chain → Sacrifice → Guild → Ascend AP → Misc
local MoveTab = Win:Tab("🚀",  "Movement")
local ESPTab  = Win:Tab("✨",  "Visuals")
local AreaTab = Win:Tab("🗺️",  "Zones")
local UpgTab  = Win:Tab("⚡",  "Upgrade Chain")
...
MiscTab:Section("UI Theme")
MiscTab:Label("Neon Suite revamp: glass panels, cleaner contrast, animated gradient fallback, and MP4 background support. If MeleeRNG_UI_Background.mp4 is missing locally, the script downloads it once from the GitHub raw URL near the top of this file.")
MiscTab:Toggle("🎥 MP4 / Neon Background", "ON = use the MP4 background if getcustomasset/getsynasset can load it; if missing, auto-downloads MeleeRNG_UI_Background.mp4 from GitHub. OFF removes the background layer.", states.uiVideoBackground, function(on)
    states.uiVideoBackground = on == true
    saveSettings()
    meleeApplyUiRevamp(states.uiVideoBackground)
    SLbl.Text = states.uiVideoBackground and ("UI background ON — " .. tostring(UI_BG_LAST_STATUS)) or "UI background OFF"
end)
MiscTab:Button("⬇️", "Download / Refresh MP4", "Forces a fresh download from UI_BG_GITHUB_RAW_URL, then reapplies the UI background.", C.accent, function()
    local ok = uiEnsureBgVideoDownloaded(true)
    meleeApplyUiRevamp(states.uiVideoBackground)
    SLbl.Text = ok and ("✓ MP4 ready — " .. tostring(UI_BG_LAST_STATUS)) or ("⚠ MP4 not ready — " .. tostring(UI_BG_LAST_STATUS))
end)
MiscTab:Button("🎨", "Re-apply UI Revamp", "Re-scans the NexusLib UI and reapplies the Neon Suite styling/background.", C.pink or C.purple, function()
    local ok = meleeApplyUiRevamp(states.uiVideoBackground)
    SLbl.Text = ok and ("✓ UI revamp reapplied — " .. tostring(UI_BG_LAST_STATUS)) or "⚠ UI root not found yet"
end)
