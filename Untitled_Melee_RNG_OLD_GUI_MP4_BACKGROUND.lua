...
-- behind the window and lightly makes large background panels transparent so
-- the MP4 can show through.
...
local function uiBgSoftenLargePanel(obj)
    ...
    if obj:GetAttribute(UI_BG_SCREEN_ATTR) then return end
    ...
    -- Minimal old-GUI change: only make big window/page backgrounds translucent.
    -- Buttons, labels, cards, toggles, sliders, and layout stay the same.
    obj.BackgroundTransparency = math.max(obj.BackgroundTransparency, 0.26)
    if obj:IsA("ScrollingFrame") then
        uiBgTrySet(obj, "ScrollBarImageTransparency", 0.12)
    end
end
...
local function meleeApplyOldGuiMp4Background(enabled)
    ...
    uiBgTrySet(video, "VideoTransparency", 0.08)
    ...
    if not dim then
        ...
        dim.BackgroundTransparency = src and 0.42 or 1
...
MiscTab:Label("Old GUI kept. This only adds your MP4 behind the NexusLib window. Upload MeleeRNG_UI_Background.mp4 beside this Lua on GitHub, or keep it in the executor workspace.")
