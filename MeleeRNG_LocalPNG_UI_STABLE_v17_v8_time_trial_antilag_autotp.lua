--[[
  Melee RNG — Untitled mellee.lua — STABLE v17 bundle
  ---------------------------------------------------------------------------
...
local states = {
    autoEquipBest  = false,
    autoEquipBestInterval = 45, -- seconds between EquipBest fires; higher = fewer weapon rebuild lag spikes
    autoAscend     = false,
    ...
    perfHardHideMobs     = false, -- hard-hide non-root mob visuals under PlayerGui; keeps HumanoidRootPart active
    mobSpawnOnPlayer     = false, -- when workspace.Mobs children appear, move their root onto your character locally
    ...
    -- GeneratedDungeon: auto-TP to newest Room_N BASE part as rooms stream in.
    dungeonAutoTp = false,
    dungeonTpOffset = 1, -- studs above BASE top surface, measured at character feet
    -- Numbered dungeon room tools: enter 1/2/3/etc and target workspace.GeneratedDungeon.Room_N.
    dungeonTargetRoomNumber = 1,
    dungeonAutoTpTargetRoom = false,
    -- Time Trial tracker: UpdateTimeTrialKills tells the client the kill goal/current kills for the next room.
    timeTrialAutoStart = false,
    -- Client-only configurable room builder. Created locally in workspace; not replicated.
    clientRoomEnabled = false,
    clientRoomWidth = 96,
    clientRoomDepth = 96,
    clientRoomHeight = 36,
    clientRoomWallThickness = 2,
    clientRoomFloorThickness = 2,
    clientRoomTransparency = 0.18,
    clientRoomCeiling = false,
    clientRoomAutoRebuild = true,
}
...
states.autoEquipBestInterval = math.clamp(math.floor(tonumber(states.autoEquipBestInterval) or 45), 15, 300)
...
states.perfHardHideMobs = states.perfHardHideMobs == true
states.mobSpawnOnPlayer = states.mobSpawnOnPlayer == true
...
states.dungeonAutoTp = states.dungeonAutoTp == true
states.dungeonTpOffset = math.clamp(tonumber(states.dungeonTpOffset) or 1, 0, 50)
states.dungeonTargetRoomNumber = math.clamp(math.floor(tonumber(states.dungeonTargetRoomNumber) or 1), 1, 1000000)
states.dungeonAutoTpTargetRoom = states.dungeonAutoTpTargetRoom == true
states.timeTrialAutoStart = states.timeTrialAutoStart == true
states.clientRoomEnabled = states.clientRoomEnabled == true
states.clientRoomWidth = math.clamp(math.floor(tonumber(states.clientRoomWidth) or 96), 16, 1000)
states.clientRoomDepth = math.clamp(math.floor(tonumber(states.clientRoomDepth) or 96), 16, 1000)
states.clientRoomHeight = math.clamp(math.floor(tonumber(states.clientRoomHeight) or 36), 8, 300)
states.clientRoomWallThickness = math.clamp(math.floor(tonumber(states.clientRoomWallThickness) or 2), 1, 40)
states.clientRoomFloorThickness = math.clamp(math.floor(tonumber(states.clientRoomFloorThickness) or 2), 1, 40)
states.clientRoomTransparency = math.clamp(tonumber(states.clientRoomTransparency) or 0.18, 0, 0.95)
states.clientRoomCeiling = states.clientRoomCeiling == true
states.clientRoomAutoRebuild = states.clientRoomAutoRebuild ~= false
...
-- Misc → Hitbox Pulse block in its own function (keeps main UI IIFE under Luau local/register limit).

-- ── GeneratedDungeon auto teleport ─────────────────────────────
-- Watches workspace.GeneratedDungeon for new Room_N models and moves the
-- character above the model's BASE part. Debounced so bulk room streaming does
-- not spam teleports or tank FPS.
local _dungeonTpConn = nil
local _dungeonTpToken = 0
local _dungeonLastRoomIndex = -1
local _dungeonTargetTpConn = nil
local _dungeonTargetTpToken = 0

local function dungeonGetFolder()
    return workspace:FindFirstChild("GeneratedDungeon")
end

local function dungeonEnsureFolder()
    local folder = dungeonGetFolder()
    if folder then return folder end
    folder = Instance.new("Folder")
    folder.Name = "GeneratedDungeon"
    folder.Parent = workspace
    return folder
end

local function dungeonRoomIndex(room)
    if not room then return nil end
    local n = tostring(room.Name):match("^Room_(%d+)$")
    return n and tonumber(n) or nil
end

local function dungeonFindBase(room)
    if not (room and room.Parent and room:IsA("Model")) then return nil end
    local pp = room.PrimaryPart
    if pp and pp:IsA("BasePart") and pp.Name == "BASE" then return pp end
    local base = room:FindFirstChild("BASE", true)
    if base and base:IsA("BasePart") then return base end
    return nil
end

local function dungeonFindLatestRoom()
    local folder = dungeonGetFolder()
    if not folder then return nil, nil end
    local best, bestIndex = nil, -1
    for _, child in ipairs(folder:GetChildren()) do
        if child:IsA("Model") then
            local idx = dungeonRoomIndex(child)
            if idx and idx > bestIndex then
                bestIndex = idx
                best = child
            end
        end
...
local function dungeonParseRoomNumber(value)
    local s = tostring(value or ""):gsub("^%s+", ""):gsub("%s+$", "")
    local n = tonumber(s:match("^Room_(%d+)$") or s:match("^(%d+)$") or s:match("(%d+)"))
    if not n then return nil end
    n = math.floor(n)
    if n < 1 then return nil end
    return math.clamp(n, 1, 1000000)
end

local function dungeonRoomNameFromNumber(value)
    local n = dungeonParseRoomNumber(value)
    return n and ("Room_" .. tostring(n)) or nil, n
end

local function dungeonFindRoomByNumber(value)
    local name, n = dungeonRoomNameFromNumber(value)
    if not name then return nil, nil end
    local folder = dungeonGetFolder()
    if not folder then return nil, n end
    local room = folder:FindFirstChild(name)
    if room and room:IsA("Model") then return room, n end
    return nil, n
end

local function dungeonSetTargetRoomNumber(value, SLbl)
    local n = dungeonParseRoomNumber(value)
    if not n then
        if SLbl then SLbl.Text = "Dungeon room number: invalid" end
        return false
    end
    states.dungeonTargetRoomNumber = n
    saveSettings()
    if SLbl then SLbl.Text = "Dungeon target room: Room_" .. tostring(n) end
    return true
end

local function dungeonCreateClientRoomNumberAtPlayer(SLbl)
    local n = dungeonParseRoomNumber(states.dungeonTargetRoomNumber) or 1
    local folder = dungeonEnsureFolder()
    local name = "Room_" .. tostring(n)
    local existing = folder:FindFirstChild(name)
    if existing and existing:IsA("Model") and existing:GetAttribute("MeleeRNG_ClientRoomNumber") ~= true then
        if SLbl then SLbl.Text = name .. " already exists; using existing real room instead of overwriting it" end
        return existing, n
    elseif existing and not existing:IsA("Model") then
    ...
    local root = getRoot()
    if not root then
        if SLbl then SLbl.Text = "Create Room_N failed: root missing" end
        return nil, n
    end
    ...
    if not room then
        ...
        room.Name = name
        room:SetAttribute("MeleeRNG_ClientRoomNumber", true)
        room.Parent = folder
    end

    for _, child in ipairs(room:GetChildren()) do
        if child.Name == "BASE" then pcall(function() child:Destroy() end) end
    end

    local hum = getHuman()
    local hip = (hum and tonumber(hum.HipHeight)) or 2
    local rootHalf = root.Size and (root.Size.Y * 0.5) or 1
    local floorY = root.Position.Y - hip - rootHalf - 0.05
    local _, yaw = root.CFrame:ToOrientation()
    local cf = CFrame.new(root.Position.X, floorY, root.Position.Z) * CFrame.Angles(0, yaw, 0)

    local base = Instance.new("Part")
    base.Name = "BASE"
    base.Anchored = true
    base.Size = Vector3.new(
        math.clamp(math.floor(tonumber(states.clientRoomWidth) or 96), 16, 1000),
        math.clamp(math.floor(tonumber(states.clientRoomFloorThickness) or 2), 1, 40),
        math.clamp(math.floor(tonumber(states.clientRoomDepth) or 96), 16, 1000)
    )
    base.CFrame = cf * CFrame.new(0, -base.Size.Y * 0.5, 0)
    base.Material = Enum.Material.SmoothPlastic
    base.Color = Color3.fromRGB(35, 39, 48)
    base.Transparency = math.clamp(tonumber(states.clientRoomTransparency) or 0.18, 0, 0.95)
    ...
    base.Parent = room
    room.PrimaryPart = base

    if SLbl then SLbl.Text = "Created local " .. name .. " with BASE" end
    return room, n
end

local function dungeonTpToRoom(room)
    if not (room and room.Parent and room:IsA("Model")) then return false, "room_missing" end

    local base = nil
    for _ = 1, 80 do
        if GEN ~= _G.MeleeRNG_Gen then return false, "stopped" end
        base = dungeonFindBase(room)
    ...
    local hum = getHuman()
    local feetOffset = tonumber(states.dungeonTpOffset) or 1
    local hip = (hum and tonumber(hum.HipHeight)) or 2
    local rootHalf = root.Size and (root.Size.Y * 0.5) or 1
    local topOffset = (base.Size.Y * 0.5) + hip + rootHalf + feetOffset
    local target = base.CFrame * CFrame.new(0, topOffset, 0)
    local _, yaw = root.CFrame:ToOrientation()
    ...
    root.CFrame = CFrame.new(target.Position) * CFrame.Angles(0, yaw, 0)
    return true, room.Name
end

local function dungeonScheduleTp(room, immediate, SLbl)
    if states.dungeonAutoTp ~= true then return end
    if not (room and room:IsA("Model")) then return end

    local idx = dungeonRoomIndex(room)
    if not idx then return end
    if idx < _dungeonLastRoomIndex then return end
    _dungeonLastRoomIndex = idx

    _dungeonTpToken = _dungeonTpToken + 1
    local token = _dungeonTpToken
    task.spawn(function()
        if not immediate then task.wait(0.08) end
        if token ~= _dungeonTpToken or states.dungeonAutoTp ~= true then return end

        local latest, latestIndex = dungeonFindLatestRoom()
        if latest and latestIndex and latestIndex >= idx then
            room = latest
        end

        local ok, why = dungeonTpToRoom(room)
        if SLbl then
            SLbl.Text = ok and ("Dungeon TP: " .. tostring(why)) or ("Dungeon TP failed: " .. tostring(why))
        end
    end)
end

local function dungeonScheduleTargetTp(room, SLbl)
    _dungeonTargetTpToken = _dungeonTargetTpToken + 1
    local token = _dungeonTargetTpToken
    task.spawn(function()
        task.wait(0.08)
        if token ~= _dungeonTargetTpToken or states.dungeonAutoTpTargetRoom ~= true then return end
        local ok, why = dungeonTpToRoom(room)
        if SLbl then
            SLbl.Text = ok and ("Dungeon target TP: " .. tostring(why)) or ("Dungeon target TP failed: " .. tostring(why))
        end
    end)
end

local function dungeonDisconnectAutoTp()
    if _dungeonTpConn then
        pcall(function() _dungeonTpConn:Disconnect() end)
        _dungeonTpConn = nil
    end
end

local function dungeonDisconnectTargetTp()
    _dungeonTargetTpToken = _dungeonTargetTpToken + 1
    if _dungeonTargetTpConn then
        pcall(function() _dungeonTargetTpConn:Disconnect() end)
        _dungeonTargetTpConn = nil
    end
end

local function dungeonSetAutoTpTargetRoom(on, SLbl)
    states.dungeonAutoTpTargetRoom = on == true
    dungeonDisconnectTargetTp()

    if states.dungeonAutoTpTargetRoom ~= true then
        if SLbl then SLbl.Text = "Dungeon target room auto TP: OFF" end
        return
    end

    local folder = dungeonGetFolder()
    local target = dungeonParseRoomNumber(states.dungeonTargetRoomNumber) or 1
    if not folder then
        if SLbl then SLbl.Text = "Dungeon target TP: waiting for workspace.GeneratedDungeon" end
        task.spawn(function()
            for _ = 1, 120 do
                if GEN ~= _G.MeleeRNG_Gen or states.dungeonAutoTpTargetRoom ~= true then return end
                folder = dungeonGetFolder()
                if folder then break end
                task.wait(0.25)
            end
            if folder and states.dungeonAutoTpTargetRoom == true then
                dungeonSetAutoTpTargetRoom(true, SLbl)
            end
        end)
        return
    end

    local existing = dungeonFindRoomByNumber(target)
    if existing then
        dungeonScheduleTargetTp(existing, SLbl)
    end

    _dungeonTargetTpConn = folder.ChildAdded:Connect(function(child)
        if states.dungeonAutoTpTargetRoom ~= true or not child:IsA("Model") then return end
        local idx = dungeonRoomIndex(child)
        if idx and idx == (dungeonParseRoomNumber(states.dungeonTargetRoomNumber) or target) then
            dungeonScheduleTargetTp(child, SLbl)
        end
    end)
    addConn(_dungeonTargetTpConn)

    if SLbl then SLbl.Text = "Dungeon target room auto TP: watching Room_" .. tostring(target) end
end

local function dungeonSetAutoTp(on, SLbl)
    states.dungeonAutoTp = on == true
    dungeonDisconnectAutoTp()

    if states.dungeonAutoTp ~= true then
        if SLbl then SLbl.Text = "Dungeon TP: OFF" end
        return
    end

    local folder = dungeonGetFolder()
    if not folder then
        if SLbl then SLbl.Text = "Dungeon TP: waiting for workspace.GeneratedDungeon" end
        task.spawn(function()
            for _ = 1, 120 do
                if GEN ~= _G.MeleeRNG_Gen or states.dungeonAutoTp ~= true then return end
                folder = dungeonGetFolder()
                if folder then break end
                task.wait(0.25)
            end
            if folder and states.dungeonAutoTp == true then
                dungeonSetAutoTp(true, SLbl)
            end
        end)
        return
    end

    local _, idx = dungeonFindLatestRoom()
    _dungeonLastRoomIndex = idx or -1
    _dungeonTpConn = folder.ChildAdded:Connect(function(child)
        if states.dungeonAutoTp == true and child:IsA("Model") then
            dungeonScheduleTp(child, false, SLbl)
        end
    end)
    addConn(_dungeonTpConn)

    if SLbl then SLbl.Text = "Dungeon TP: ON — watching new Room_N models" end
end

local function meleeGuiMountDungeonTpSection(MiscTab, SLbl, C)
    MiscTab:Section("GeneratedDungeon")
    MiscTab:Label("Auto-teleports to the newest workspace.GeneratedDungeon.Room_N model when it appears, landing 1 stud above its BASE top surface.")
    MiscTab:Toggle("🏰 Auto TP new rooms", "Watches GeneratedDungeon.ChildAdded; teleports to new Room_N → BASE. Saved.", states.dungeonAutoTp, function(on)
        dungeonSetAutoTp(on == true, SLbl)
        saveSettings()
    end)
    MiscTab:Input(
        "Room number",
        "Enter 1, 2, 3, or Room_3. Used by the target-room buttons below. Saved.",
        "room number",
        tostring(states.dungeonTargetRoomNumber or 1),
        function(txt)
            dungeonSetTargetRoomNumber(txt, SLbl)
            if states.dungeonAutoTpTargetRoom == true then
                dungeonSetAutoTpTargetRoom(true, SLbl)
            end
        end
    )
    MiscTab:Toggle("🎯 Auto TP entered Room_N", "Only teleports when the room number you entered appears, instead of always using the latest room. Saved.", states.dungeonAutoTpTargetRoom, function(on)
        dungeonSetAutoTpTargetRoom(on == true, SLbl)
        saveSettings()
    end)
    MiscTab:Button("➕", "Create local entered Room_N BASE", "Creates workspace.GeneratedDungeon.Room_<number> locally with a BASE part at your feet. It will not overwrite an existing real Room_N model.", C.green, function()
        dungeonCreateClientRoomNumberAtPlayer(SLbl)
    end)
    MiscTab:Button("🎯", "TP entered Room_N now", "Finds the room number you entered and teleports above its BASE once.", C.purple, function()
        local room, n = dungeonFindRoomByNumber(states.dungeonTargetRoomNumber)
        local ok, why = dungeonTpToRoom(room)
        SLbl.Text = ok and ("Dungeon TP: Room_" .. tostring(n) .. " / " .. tostring(why)) or ("Dungeon TP failed for Room_" .. tostring(n or "?") .. ": " .. tostring(why))
    end)
    MiscTab:Button("⏩", "TP latest room now", "Finds highest Room_N and teleports above BASE once.", C.accent, function()
        local room = dungeonFindLatestRoom()
        local ok, why = dungeonTpToRoom(room)
        SLbl.Text = ok and ("Dungeon TP: " .. tostring(why)) or ("Dungeon TP failed: " .. tostring(why))
    end)
    task.defer(function()
...
-- ── Time Trial / UpdateTimeTrialKills tracker ─────────────────
-- Uses the existing Remotes folder only. UpdateTimeTrialKills is treated as the
-- server/client update carrying the current/required kill amount for room progress.
-- This module only observes and starts the normal time trial flow; it does not
-- create fake server progress or local-only room progress.
local _ttKillsConn = nil
local _ttStatusLbl = nil
local _ttLastRequiredKills = 0
local _ttLastCurrentKills = 0
local _ttLastUpdateClock = 0
local _ttLastArgsText = "waiting"
local _ttLastTimeLeft = nil
local _ttStartLoopToken = 0

local function ttNumber(v)
    local n = tonumber(v)
...
local function ttShortValue(v, depth)
    depth = depth or 0
    local tv = typeof(v)
    if tv == "number" or tv == "boolean" or tv == "nil" then
        return tostring(v)
    elseif tv == "string" then
        if #v > 32 then return string.sub(v, 1, 29) .. "..." end
    ...
    elseif tv == "table" then
        ...
        for k, val in pairs(v) do
            count += 1
            if count > 5 then bits[#bits + 1] = "..."; break end
            bits[#bits + 1] = tostring(k) .. "=" .. ttShortValue(val, depth + 1)
        end
        table.sort(bits)
...
local function ttArgsToText(pack)
    local bits = {}
    for i = 1, math.min(pack.n or #pack, 6) do
        bits[#bits + 1] = ttShortValue(pack[i], 0)
    end
    return table.concat(bits, " | ")
end

local function ttFindNumberByNames(tbl, names)
    ...
    for k, v in pairs(tbl) do
        if type(k) == "string" then
            ...
            for _, key in ipairs(names) do
                ...
                if lk == want or string.find(lk, want, 1, true) then
                    local n = ttNumber(v)
                    if n then return n end
                end
            end
...
local function ttParseKillUpdate(...)
    local pack = table.pack(...)
    _ttLastArgsText = ttArgsToText(pack)
    _ttLastUpdateClock = os.clock()
    ...
    for i = 1, pack.n do
        ...
        if type(v) == "table" then
            required = required or ttFindNumberByNames(v, {"RequiredKills", "KillsRequired", "Required", "Need", "Needed", "Goal", "Max", "Total", "Amount", "KillsNeeded"})
            current = current or ttFindNumberByNames(v, {"CurrentKills", "Kills", "Current", "Progress", "Count", "Done"})
        end
    end

    local nums = {}
    for i = 1, pack.n do
        local n = ttNumber(pack[i])
        if n then nums[#nums + 1] = n end
    end

    if not required and #nums == 1 then
        -- You confirmed UpdateTimeTrialKills can be a single required-kill amount.
        required = nums[1]
    elseif not required and #nums >= 2 then
        -- Common shape: current, required OR required, current. Use the larger as goal.
        local a, b = nums[1], nums[2]
        required = math.max(a, b)
        current = math.min(a, b)
    elseif required and not current and #nums >= 2 then
        for _, n in ipairs(nums) do
            if n ~= required then current = n; break end
        end
    end

    if required and required >= 0 then _ttLastRequiredKills = required end
    if current and current >= 0 then _ttLastCurrentKills = current end
end

local function ttStatusText()
    local req = (_ttLastRequiredKills and _ttLastRequiredKills > 0) and tostring(_ttLastRequiredKills) or "?"
    local cur = (_ttLastCurrentKills and _ttLastCurrentKills > 0) and tostring(_ttLastCurrentKills) or "?"
    local age = _ttLastUpdateClock > 0 and string.format("%.1fs ago", os.clock() - _ttLastUpdateClock) or "never"
    local left = _ttLastTimeLeft ~= nil and (" · time left: " .. tostring(_ttLastTimeLeft)) or ""
    return "TimeTrial kills: " .. cur .. " / " .. req .. " · update: " .. age .. left
end

local function ttRefreshStatus(extra)
    if _ttStatusLbl and _ttStatusLbl.Set then
        _ttStatusLbl.Set(ttStatusText() .. (extra and (" · " .. tostring(extra)) or ""))
    end
end

local function ttConnectUpdateKills(SLbl)
    if _ttKillsConn then return true end
    local r = R("UpdateTimeTrialKills")
    if not r then
        if SLbl then SLbl.Text = "TimeTrial: UpdateTimeTrialKills remote missing" end
        ttRefreshStatus("remote missing")
        return false
    end
    if r:IsA("RemoteEvent") then
        _ttKillsConn = r.OnClientEvent:Connect(function(...)
            ttParseKillUpdate(...)
            ttRefreshStatus()
        end)
        addConn(_ttKillsConn)
        ttRefreshStatus("listening")
        if SLbl then SLbl.Text = "TimeTrial: listening to UpdateTimeTrialKills" end
        return true
    elseif r:IsA("RemoteFunction") then
        ttRefreshStatus("RemoteFunction")
        if SLbl then SLbl.Text = "TimeTrial: UpdateTimeTrialKills is a RemoteFunction" end
        return true
    end
    ttRefreshStatus(r.ClassName)
    if SLbl then SLbl.Text = "TimeTrial: UpdateTimeTrialKills is " .. tostring(r.ClassName) end
    return false
end

local function ttStartTrial(SLbl)
    ttConnectUpdateKills(SLbl)
    local r = R("StartTimeTrial")
    if not r then
        if SLbl then SLbl.Text = "TimeTrial: StartTimeTrial remote missing" end
        return false
    end
    local ok, res
    if r:IsA("RemoteFunction") then
        res = invokeR("StartTimeTrial")
        ok = true
    elseif r:IsA("RemoteEvent") then
        ok = fireR("StartTimeTrial") == true
    else
        ok = false
    end
    if SLbl then
        SLbl.Text = ok and "TimeTrial: StartTimeTrial sent" or ("TimeTrial: StartTimeTrial failed (" .. tostring(r.ClassName) .. ")")
    end
    return ok, res
end

local function ttRefreshRemoteInfo(SLbl)
    ttConnectUpdateKills(SLbl)
    local left = invokeR("GetUpdateTimeLeft")
    if left ~= nil then _ttLastTimeLeft = left end
    local spawns = invokeR("GetPossibleSpawnPositionsForTimeTrial")
    local spawnCount = 0
    if type(spawns) == "table" then
        for _ in pairs(spawns) do spawnCount += 1 end
    end
    ttRefreshStatus("spawns=" .. tostring(spawnCount))
    if SLbl then
        SLbl.Text = "TimeTrial info: " .. ttStatusText() .. " · spawns=" .. tostring(spawnCount)
    end
    print("[MeleeRNG/TimeTrial] UpdateTimeTrialKills args:", _ttLastArgsText)
    print("[MeleeRNG/TimeTrial] GetUpdateTimeLeft:", ttShortValue(left))
    print("[MeleeRNG/TimeTrial] GetPossibleSpawnPositionsForTimeTrial count:", spawnCount, "raw:", ttShortValue(spawns))
end

local function ttSetAutoStart(on, SLbl)
    states.timeTrialAutoStart = on == true
    saveSettings()
    _ttStartLoopToken += 1
    if states.timeTrialAutoStart ~= true then
        if SLbl then SLbl.Text = "TimeTrial auto-start: OFF" end
        return
    end
    local token = _ttStartLoopToken
    ttConnectUpdateKills(SLbl)
    task.spawn(function()
        while GEN == _G.MeleeRNG_Gen and token == _ttStartLoopToken and states.timeTrialAutoStart == true do
            ttStartTrial(SLbl)
            task.wait(12)
        end
    end)
    if SLbl then SLbl.Text = "TimeTrial auto-start: ON" end
end

local function meleeGuiMountTimeTrialSection(MiscTab, SLbl, C)
    MiscTab:Section("Time Trial")
    MiscTab:Label("UpdateTimeTrialKills is tracked as the current/required kill amount for room progress. The code listens to it and displays the goal.")
    _ttStatusLbl = MiscTab:Label(ttStatusText())
    MiscTab:Toggle("⏱️ Auto Start Time Trial", "Every ~12s sends the existing StartTimeTrial remote and listens for UpdateTimeTrialKills. Saved.", states.timeTrialAutoStart, function(on)
        ttSetAutoStart(on == true, SLbl)
    end)
    MiscTab:Button("▶️", "Start Time Trial once", "Sends existing StartTimeTrial remote once and starts listening for UpdateTimeTrialKills.", C.green, function()
        ttStartTrial(SLbl)
    end)
    MiscTab:Button("🔄", "Refresh Time Trial info", "Invokes GetUpdateTimeLeft/GetPossibleSpawnPositionsForTimeTrial and prints last UpdateTimeTrialKills args.", C.accent, function()
        ttRefreshRemoteInfo(SLbl)
    end)
    MiscTab:Button("🧪", "Print Time Trial remotes", "Prints ClassName for the Time Trial remotes you showed.", C.purple, function()
        local names = {"StartTimeTrial", "SpawnMob", "UpdateTimeTrialKills", "GetUpdateTimeLeft", "GetPossibleSpawnPositionsForTimeTrial", "UpdateStartPage"}
        for _, name in ipairs(names) do
            local r = R(name)
            print("[MeleeRNG/TimeTrial]", name, r and r.ClassName or "MISSING")
        end
        print("[MeleeRNG/TimeTrial] last UpdateTimeTrialKills args:", _ttLastArgsText)
        if SLbl then SLbl.Text = "TimeTrial remotes printed to console" end
    end)
    task.defer(function()
        ttConnectUpdateKills(SLbl)
        if states.timeTrialAutoStart == true then
            ttSetAutoStart(true, SLbl)
        end
    end)
end


-- ── Client-only configurable room builder ─────────────────────
-- Builds a small local-only room in workspace. It is intentionally only 5–6
-- anchored parts (floor + walls + optional ceiling), with CanTouch/CanQuery off
-- to avoid adding much overhead while still giving you a configurable space.
local CLIENT_ROOM_NAME = "MeleeRNG_Client_ConfigRoom"
local _clientRoomLastFloorCFrame = nil

local function clientRoomGetModel()
    local m = workspace:FindFirstChild(CLIENT_ROOM_NAME)
    if m and not m:IsA("Model") then
        m:Destroy()
        m = nil
    end
    if not m then
        m = Instance.new("Model")
        m.Name = CLIENT_ROOM_NAME
        m.Parent = workspace
    end
    return m
end

local function clientRoomClear()
    local m = workspace:FindFirstChild(CLIENT_ROOM_NAME)
    if m then
...
local function clientRoomColor(name)
    if name == "Floor" then return Color3.fromRGB(35, 39, 48) end
    if name == "Ceiling" then return Color3.fromRGB(20, 24, 32) end
    return Color3.fromRGB(18, 22, 31)
end

local function clientRoomMakePart(parent, name, size, cf)
    ...
    part.Color = clientRoomColor(name)
    part.Transparency = states.clientRoomTransparency
...
local function clientRoomFloorCFrameAtPlayer()
    local root = getRoot()
    if not root then return nil end
    local hum = getHuman()
    local hip = (hum and tonumber(hum.HipHeight)) or 2
    ...
    local floorY = root.Position.Y - hip - rootHalf - 0.05
    local _, yaw = root.CFrame:ToOrientation()
    return CFrame.new(root.Position.X, floorY, root.Position.Z) * CFrame.Angles(0, yaw, 0)
end

local function clientRoomFloorCFrameAtLatestDungeonBase()
    local room = dungeonFindLatestRoom()
    local base = dungeonFindBase(room)
    if not base then return nil, "no_latest_base" end
    local top = base.CFrame * CFrame.new(0, base.Size.Y * 0.5 + 0.05, 0)
    return top, room and room.Name or "latest"
end

local function clientRoomFloorCFrameAtTargetDungeonBase()
    local room, n = dungeonFindRoomByNumber(states.dungeonTargetRoomNumber)
    local base = dungeonFindBase(room)
    if not base then return nil, "no_Room_" .. tostring(n or "?") .. "_BASE" end
    local top = base.CFrame * CFrame.new(0, base.Size.Y * 0.5 + 0.05, 0)
    return top, room and room.Name or ("Room_" .. tostring(n))
end

local function clientRoomBuildAtFloor(floorCf, SLbl, reason)
    if not floorCf then
        if SLbl then SLbl.Text = "Client room: no valid build position" end
        return false
    end

    _clientRoomLastFloorCFrame = floorCf
    clientRoomClear()

    local w = math.clamp(math.floor(tonumber(states.clientRoomWidth) or 96), 16, 1000)
    local d = math.clamp(math.floor(tonumber(states.clientRoomDepth) or 96), 16, 1000)
    local h = math.clamp(math.floor(tonumber(states.clientRoomHeight) or 36), 8, 300)
    local wt = math.clamp(math.floor(tonumber(states.clientRoomWallThickness) or 2), 1, 40)
    local ft = math.clamp(math.floor(tonumber(states.clientRoomFloorThickness) or 2), 1, 40)

    local m = clientRoomGetModel()
    clientRoomMakePart(m, "Floor", Vector3.new(w, ft, d), floorCf * CFrame.new(0, -ft * 0.5, 0))
    clientRoomMakePart(m, "Wall_North", Vector3.new(w + wt * 2, h, wt), floorCf * CFrame.new(0, h * 0.5, -d * 0.5 - wt * 0.5))
    clientRoomMakePart(m, "Wall_South", Vector3.new(w + wt * 2, h, wt), floorCf * CFrame.new(0, h * 0.5, d * 0.5 + wt * 0.5))
    clientRoomMakePart(m, "Wall_East", Vector3.new(wt, h, d), floorCf * CFrame.new(w * 0.5 + wt * 0.5, h * 0.5, 0))
    clientRoomMakePart(m, "Wall_West", Vector3.new(wt, h, d), floorCf * CFrame.new(-w * 0.5 - wt * 0.5, h * 0.5, 0))
    if states.clientRoomCeiling == true then
        clientRoomMakePart(m, "Ceiling", Vector3.new(w, ft, d), floorCf * CFrame.new(0, h + ft * 0.5, 0))
    end
    m.PrimaryPart = m:FindFirstChild("Floor")

    if SLbl then
        SLbl.Text = "Client room built" .. (reason and (" at " .. tostring(reason)) or "")
    end
    return true
end

local function clientRoomRebuildIfNeeded(SLbl)
    if states.clientRoomEnabled ~= true then return end
    local cf = _clientRoomLastFloorCFrame or clientRoomFloorCFrameAtPlayer()
    clientRoomBuildAtFloor(cf, SLbl, "saved position")
end

local function clientRoomSetEnabled(on, SLbl)
    states.clientRoomEnabled = on == true
    if states.clientRoomEnabled then
        clientRoomBuildAtFloor(_clientRoomLastFloorCFrame or clientRoomFloorCFrameAtPlayer(), SLbl, "you")
    else
        clientRoomClear()
        if SLbl then SLbl.Text = "Client room cleared" end
    end
end

local function clientRoomTeleportInside(SLbl)
    local floor = workspace:FindFirstChild(CLIENT_ROOM_NAME)
    floor = floor and floor:FindFirstChild("Floor")
    local root = getRoot()
    if not floor or not root then
        if SLbl then SLbl.Text = "Client room TP failed: room/root missing" end
        return
    end
    local hum = getHuman()
    local hip = (hum and tonumber(hum.HipHeight)) or 2
    local rootHalf = root.Size and (root.Size.Y * 0.5) or 1
    root.CFrame = floor.CFrame * CFrame.new(0, floor.Size.Y * 0.5 + hip + rootHalf + 1, 0)
    if SLbl then SLbl.Text = "Teleported inside client room" end
end

local function meleeGuiMountClientRoomSection(MiscTab, SLbl, C)
    MiscTab:Section("Client Config Room")
    MiscTab:Label("Creates a local-only room in workspace. It is only floor/walls/optional ceiling, so it is much lighter than cloning dungeon rooms.")
    MiscTab:Toggle("🏗️ Client room enabled", "Builds/clears the local room. Saved.", states.clientRoomEnabled, function(on)
        clientRoomSetEnabled(on == true, SLbl)
        saveSettings()
    end)
    MiscTab:Toggle("🔁 Rebuild room on slider changes", "When ON, size/transparency sliders instantly rebuild the room. Saved.", states.clientRoomAutoRebuild, function(on)
        states.clientRoomAutoRebuild = on == true
        saveSettings()
    end)
    MiscTab:Toggle("🧱 Add ceiling", "Optional ceiling part. Leave OFF for slightly less rendering/physics work.", states.clientRoomCeiling, function(on)
        states.clientRoomCeiling = on == true
        saveSettings()
        if states.clientRoomAutoRebuild then clientRoomRebuildIfNeeded(SLbl) end
    end)
    MiscTab:Slider("Room width", "Studs on local X axis", {min=16, max=300, default=states.clientRoomWidth, format=function(v) return math.floor(v) .. " studs" end}, function(v)
        states.clientRoomWidth = math.clamp(math.floor(tonumber(v) or 96), 16, 1000)
        saveSettings()
        if states.clientRoomAutoRebuild then clientRoomRebuildIfNeeded(SLbl) end
    end)
    MiscTab:Slider("Room depth", "Studs on local Z axis", {min=16, max=300, default=states.clientRoomDepth, format=function(v) return math.floor(v) .. " studs" end}, function(v)
        states.clientRoomDepth = math.clamp(math.floor(tonumber(v) or 96), 16, 1000)
        saveSettings()
        if states.clientRoomAutoRebuild then clientRoomRebuildIfNeeded(SLbl) end
    end)
    MiscTab:Slider("Room height", "Wall height in studs", {min=8, max=120, default=states.clientRoomHeight, format=function(v) return math.floor(v) .. " studs" end}, function(v)
        states.clientRoomHeight = math.clamp(math.floor(tonumber(v) or 36), 8, 300)
        saveSettings()
        if states.clientRoomAutoRebuild then clientRoomRebuildIfNeeded(SLbl) end
    end)
    MiscTab:Slider("Wall thickness", "Bigger walls are easier to see; smaller walls are lighter", {min=1, max=12, default=states.clientRoomWallThickness, format=function(v) return math.floor(v) .. " studs" end}, function(v)
        states.clientRoomWallThickness = math.clamp(math.floor(tonumber(v) or 2), 1, 40)
        saveSettings()
        if states.clientRoomAutoRebuild then clientRoomRebuildIfNeeded(SLbl) end
    end)
    MiscTab:Slider("Room transparency", "0 = solid, 95 = almost invisible", {min=0, max=95, default=math.floor((states.clientRoomTransparency or 0.18) * 100), format=function(v) return tostring(math.floor(v)) .. "%" end}, function(v)
        states.clientRoomTransparency = math.clamp((tonumber(v) or 18) / 100, 0, 0.95)
        saveSettings()
        if states.clientRoomAutoRebuild then clientRoomRebuildIfNeeded(SLbl) end
    end)
    MiscTab:Button("📍", "Build room at me", "Builds the room under your current character position.", C.accent, function()
        states.clientRoomEnabled = true
        clientRoomBuildAtFloor(clientRoomFloorCFrameAtPlayer(), SLbl, "you")
        saveSettings()
    end)
    MiscTab:Button("🔢", "Build room at entered Room_N BASE", "Uses the room number from GeneratedDungeon → Room number.", C.purple, function()
        local cf, why = clientRoomFloorCFrameAtTargetDungeonBase()
        if cf then
            states.clientRoomEnabled = true
            clientRoomBuildAtFloor(cf, SLbl, tostring(why))
            saveSettings()
        else
            SLbl.Text = "Client room failed: " .. tostring(why)
        end
    end)
    MiscTab:Button("🏰", "Build room at latest dungeon BASE", "Builds the room above the newest GeneratedDungeon Room_N BASE.", C.green, function()
        local cf, why = clientRoomFloorCFrameAtLatestDungeonBase()
        if cf then
            states.clientRoomEnabled = true
            clientRoomBuildAtFloor(cf, SLbl, tostring(why))
            saveSettings()
    ...
    MiscTab:Button("🧍", "TP inside client room", "Teleports you onto the room floor.", C.purple, function()
        clientRoomTeleportInside(SLbl)
    end)
    MiscTab:Button("🧹", "Clear client room", "Destroys the local room model.", C.red, function()
        states.clientRoomEnabled = false
        clientRoomClear()
        saveSettings()
        SLbl.Text = "Client room cleared"
    end)
    task.defer(function()
        if states.clientRoomEnabled == true then
            clientRoomBuildAtFloor(_clientRoomLastFloorCFrame or clientRoomFloorCFrameAtPlayer(), SLbl, "auto-load")
        end
    end)
end

local function meleeGuiMountHitboxPulseSection(MoveTab, SLbl, C)
    -- --------------------------------------------------------------------------
    -- Misc tab — Hitbox pulse (lag-safe local pulse + touch assist)
    -- Old path resized every sword every Heartbeat phase. That makes overlap work scale
    -- with weapon count. This version pulses a small rolling batch, keeps only a few
    -- hitboxes BIG at once, and optionally nudges local Touched listeners for reliability.
    ...
    local HITBOX_DUTY_MAX     = 20
    local HITBOX_BATCH_MAX    = 8     -- hard cap: more swords no longer means every sword is resized in one frame
    local HITBOX_HOLD_FRAMES  = 2     -- keep active batch BIG long enough for physics/Touched to catch it
    local HITBOX_MOB_SCAN_SEC = 0.18  -- throttle workspace.Mobs scans
    local HITBOX_TOUCH_ASSIST_BUDGET = 384 -- bounded local touch nudges per pulse; lower = less lag
    local HITBOX_TOUCHES_PER_TARGET = 12   -- each mob gets up to this many weapon-hit nudges per pulse before rolling onward
    ...
    local _hitboxCache    = {}
    local _hitboxSet      = {}
    local _pulseHot       = {} -- [BasePart] = frames remaining before shrinking
    local _pulseCursor    = 1
    local _pulseBigVec    = Vector3.new(hitboxSize, hitboxSize, hitboxSize)
    local _pulseMobTargets = {}
    local _pulseNextMobScan = 0
    local _pulseLastStatusAt = 0
    local _touchAssistHitboxCursor = 1
    local _touchAssistTargetCursor = 1

    local function resolveFn(name)
        local v = rawget(_G, name)
        if type(v) == "function" then return v end
        local ok, envVal = pcall(function()
            local env = getfenv and getfenv(0)
            return env and env[name]
        end)
        if ok and type(envVal) == "function" then return envVal end
        ok, envVal = pcall(function()
    ...
    local _firetouchinterest = resolveFn("firetouchinterest")
    local _firesignal = resolveFn("firesignal")
    local _hitboxGetConnections = _getconnections
    if type(_hitboxGetConnections) ~= "function" then
        _hitboxGetConnections = resolveFn("getconnections")
    end

    local function touchApiStatus()
        local a = type(_hitboxGetConnections) == "function" and "conn" or "-"
        local b = type(_firesignal) == "function" and "signal" or "-"
        local c = type(_firetouchinterest) == "function" and "fti" or "-"
        return a .. "/" .. b .. "/" .. c
    end

    MoveTab:Section("Hitbox Pulse")
    MoveTab:Label("Lag-safe pulse: only a small rolling batch of weapon Hitboxes goes BIG each cycle, instead of resizing every sword at once.")
    MoveTab:Label("Reliability: BIG is held for 2 frames, globals are resolved correctly, and each mob gets a burst of local Touched nudges for one-shot behavior.")
    MoveTab:Label("Duty N: every N Heartbeats, the next batch pulses BIG; higher N = calmer physics. Batch cap is fixed in code so more swords does not spike one frame.")
    MoveTab:Label("Slider = BIG edge length. Keep it near 350–500 for best KPM; huge values create overlap hell and can reduce kills.")

    local hitboxDutySlider, hitboxRadiusSlider

    local function currentHitboxDuty()
        return math.clamp(math.floor(tonumber(states.hitboxDutyCycle) or 2), 2, HITBOX_DUTY_MAX)
    end

    local function currentHitboxBatchSize(totalHitboxes)
        totalHitboxes = math.max(0, math.floor(tonumber(totalHitboxes) or 0))
        if totalHitboxes <= 0 then return 0 end
        -- Pulse all if the player only has a few. Cap once weapon count gets high.
        return math.min(totalHitboxes, HITBOX_BATCH_MAX)
    end

    hitboxDutySlider = MoveTab:Slider("Big phase duty (1 batch / N heartbeats)", "Every N Heartbeats, only the next capped batch goes BIG. Higher N = less physics/Touched pressure.",
        {min=2, max=HITBOX_DUTY_MAX, default=currentHitboxDuty(),
         format=function(v) return "1 batch / "..tostring(math.floor(v)) end},
        function(v)
            states.hitboxDutyCycle = math.clamp(math.floor(v), 2, HITBOX_DUTY_MAX)
            saveSettings()
        end)

    hitboxRadiusSlider = MoveTab:Slider("BIG size (studs)", "Edge length of the active batch. ~500 is the sweet spot; very large values often lower KPM by overloading physics.",
    ...
    MoveTab:Button("⚡", "Max farm — duty 2", "BIG 500 · fastest batch pulse without resizing all swords at once", C.green, function()
        applyHitboxPulsePreset("Max farm — duty 2", 2, HITBOX_BIG_DEFAULT)
    end)
    MoveTab:Button("⚖️", "Balanced — duty 5", "BIG 500 · same reach as ⚡ with much calmer physics", C.accent, function()
        applyHitboxPulsePreset("Balanced — duty 5", 5, HITBOX_BIG_DEFAULT)
    end)
    MoveTab:Button("🐢", "Low lag — duty 10", "BIG 350 · smaller footprint + rare BIG batches", C.sub, function()
            ...
            if weapon:IsA("Model") then
                for _, desc in pairs(weapon:GetDescendants()) do
                    if desc:IsA("BasePart") and desc.Name == "Hitbox" then
                        ...
                        _hitboxSet[desc] = true
                    end
                end
            end
        end

        for hb in pairs(_pulseHot) do
            if not _hitboxSet[hb] then
                _pulseHot[hb] = nil
            end
        end
        if _pulseCursor > #_hitboxCache then _pulseCursor = 1 end
        _hitboxCacheDirty = false
    end

    local function getPulseMobTargets()
        local now = os.clock()
        if now < _pulseNextMobScan then
            return _pulseMobTargets
        end
        _pulseNextMobScan = now + HITBOX_MOB_SCAN_SEC
        ...
        _pulseMobTargets = targets
        return targets
    end

    local function fireLocalTouched(hitbox, otherPart)
        if not (hitbox and hitbox.Parent and otherPart and otherPart.Parent) then return false end

        local fired = false

        -- Preferred: call the local weapon Hitbox.Touched listeners directly.
        -- This avoids relying on physics contact replication and is much more reliable for local-only one-shot.
        if type(_hitboxGetConnections) == "function" then
            local ok, conns = pcall(_hitboxGetConnections, hitbox.Touched)
            if ok and type(conns) == "table" then
                for _, c in ipairs(conns) do
                    local fn = type(c) == "table" and c.Function or nil
        ...
        -- Some environments support firesignal globally but not through _G.
        if type(_firesignal) == "function" then
            local ok = pcall(_firesignal, hitbox.Touched, otherPart)
            fired = fired or ok
        end

        -- Physics fallback. Delay the release by one frame so TouchStarted/TouchEnded do not collapse into the same solver step.
        if type(_firetouchinterest) == "function" then
            local ok0 = pcall(_firetouchinterest, hitbox, otherPart, 0)
            fired = fired or ok0
            task.defer(function()
                pcall(_firetouchinterest, hitbox, otherPart, 1)
            end)
        end
    ...
    local function pulseTouchAssist(activeHitboxes)
        if type(_firetouchinterest) ~= "function" and type(_firesignal) ~= "function" and type(_hitboxGetConnections) ~= "function" then
            return 0
        end

        local targets = getPulseMobTargets()
        local hitboxes = _hitboxCache
        local targetCount = targets and #targets or 0
        local hitboxCount = hitboxes and #hitboxes or 0
        if targetCount == 0 or hitboxCount == 0 then return 0 end

        local maxDist = math.max(20, tonumber(hitboxSize) or HITBOX_BIG_DEFAULT) + 35
        local firedCount = 0
        local checked = 0
        local budget = math.max(HITBOX_TOUCH_ASSIST_BUDGET, math.min(2048, targetCount * HITBOX_TOUCHES_PER_TARGET))
        local maxChecks = math.max(budget * 3, hitboxCount * math.min(targetCount, 8))

        -- One-shot assist: target-first burst. Each mob gets several different
        -- weapon hitboxes nudged immediately, so high-health mobs do not wait for
        -- the slow rolling cursor to eventually revisit them.
        for ti = 1, targetCount do
            local target = targets[ti]
            local perTarget = 0
            if target and target.Parent then
                local start = _touchAssistHitboxCursor
                local seen = 0
                while perTarget < HITBOX_TOUCHES_PER_TARGET and seen < hitboxCount and firedCount < budget do
                    seen = seen + 1
                    if _touchAssistHitboxCursor > hitboxCount then _touchAssistHitboxCursor = 1 end
                    local hb = hitboxes[_touchAssistHitboxCursor]
                    _touchAssistHitboxCursor = _touchAssistHitboxCursor + 1

                    if hb and hb.Parent then
                        if (target.Position - hb.Position).Magnitude <= maxDist then
                            if fireLocalTouched(hb, target) then
                                firedCount = firedCount + 1
                                perTarget = perTarget + 1
                            end
                        end
                    else
                        _hitboxCacheDirty = true
                    ...
                    if _touchAssistHitboxCursor == start then
                        -- Wrapped all hitboxes once for this target.
                        break
                    end
                end
            end
            if firedCount >= budget then
                return firedCount
            end
        end

        -- Also nudge the physically BIG batch, because some local weapon code may
        -- expect the active resized part specifically.
        if activeHitboxes and #activeHitboxes > 0 then
            for hi = 1, #activeHitboxes do
                local hb = activeHitboxes[hi]
                if hb and hb.Parent then
                    local hbPos = hb.Position
                    for ti = 1, targetCount do
                        local target = targets[ti]
                        if target and target.Parent and (target.Position - hbPos).Magnitude <= maxDist then
                            if fireLocalTouched(hb, target) then
                                firedCount = firedCount + 1
                            end
                            if firedCount >= budget then
        ...
        -- Final rolling fill across all pairs, but bounded, so FPS does not explode.
        if _touchAssistHitboxCursor > hitboxCount then _touchAssistHitboxCursor = 1 end
        if _touchAssistTargetCursor > targetCount then _touchAssistTargetCursor = 1 end

        while firedCount < budget and checked < maxChecks do
            checked = checked + 1

            local hb = hitboxes[_touchAssistHitboxCursor]
            local target = targets[_touchAssistTargetCursor]

            _touchAssistTargetCursor = _touchAssistTargetCursor + 1
            if _touchAssistTargetCursor > targetCount then
                _touchAssistTargetCursor = 1
                _touchAssistHitboxCursor = _touchAssistHitboxCursor + 1
                if _touchAssistHitboxCursor > hitboxCount then
                    _touchAssistHitboxCursor = 1
                end
            end

            if hb and hb.Parent and target and target.Parent then
                if (target.Position - hb.Position).Magnitude <= maxDist then
                    if fireLocalTouched(hb, target) then
                        firedCount = firedCount + 1
                    end
    ...
    local function shrinkExpiredPulseHitboxes()
        for hb, framesLeft in pairs(_pulseHot) do
            if not (hb and hb.Parent) then
                ...
                if framesLeft <= 0 then
                    if hb.Size ~= _TINY_V then hb.Size = _TINY_V end
                    _pulseHot[hb] = nil
                else
                    _pulseHot[hb] = framesLeft
                end
            end
        end
    end

    local function activateNextPulseBatch()
        local cache = _hitboxCache
        local total = #cache
        if total <= 0 then return {}, 0 end

        local batchSize = currentHitboxBatchSize(total)
        local active = {}
        local attempts = 0

        while #active < batchSize and attempts < total do
            attempts = attempts + 1
            if _pulseCursor > total then _pulseCursor = 1 end
            local hb = cache[_pulseCursor]
            _pulseCursor = _pulseCursor + 1

            if hb and hb.Parent then
                -- Force an edge even when a tiny write was missed: tiny -> BIG.
                if hb.Size ~= _TINY_V then hb.Size = _TINY_V end
                hb.Size = _pulseBigVec
                _pulseHot[hb] = HITBOX_HOLD_FRAMES
                active[#active + 1] = hb
            else
                _hitboxCacheDirty = true
            end
    ...
    -- Equip/unequip: child count changes → dirty → next pulse rebuildHitboxCache.
    -- This is intentionally cheap: no full descendant scan every frame.
    addThread(task.spawn(function()
        local lastCount = -1
        while true do
            if GEN ~= _G.MeleeRNG_Gen then break end
            task.wait(_pulseActive and 0.5 or 2)
            local folder = getPlayerWeaponFolder()
    ...
    local function setHitboxPulse(on)
        _pulseActive = on == true
        hitboxExpanded = _pulseActive
        ...
        if _pulseActive then
            ...
            _pulseConn = RunService.Heartbeat:Connect(function()
                ...
                shrinkExpiredPulseHitboxes()

                dutyTick = dutyTick + 1
                local duty = currentHitboxDuty()
                _pulsePhase = ((dutyTick - 1) % duty) == 0

                local activeNow = nil
                local touched = 0
                if _pulsePhase then
                    activeNow = activateNextPulseBatch()
                    touched = pulseTouchAssist(activeNow)
                end

                local now = os.clock()
                if now - _pulseLastStatusAt >= 1.0 then
                    _pulseLastStatusAt = now
                    local total = #_hitboxCache
                    local batch = currentHitboxBatchSize(total)
                    SLbl.Text = string.format(
                        "⚡ Pulse ON | %.0f studs | %d hitboxes | batch %d | duty 1/%d | touches %d | api %s",
                        hitboxSize, total, batch, duty, touched, touchApiStatus()
                    )
                end
            end)
            ...
            rebuildHitboxCache()
            SLbl.Text = string.format(
                "⚡ Pulse ON | %.0f studs | %d hitboxes | batch %d | duty 1/%d | api %s",
                hitboxSize, #_hitboxCache, currentHitboxBatchSize(#_hitboxCache), currentHitboxDuty(), touchApiStatus()
            )
        else
            if _pulseConn then _pulseConn:Disconnect(); _pulseConn = nil end
            restoreHitboxes()
    ...
    local hitboxPulseToggle = MoveTab:Toggle("⚡ Hitbox Pulse", "Lag-safe rolling BIG/tiny pulse. Uses capped batches + local Touched assist when available.", states.hitboxPulse, function(on)
    ...
    MoveTab:Button("🧪", "Print Hitbox Touch APIs", "Shows which local touch helpers are available; conn/signal/fti should not all be missing.", C.sub, function()
        print("[MeleeRNG] Hitbox Touch APIs: " .. touchApiStatus())
        print("[MeleeRNG] getconnections:", tostring(type(_hitboxGetConnections)))
        print("[MeleeRNG] firesignal:", tostring(type(_firesignal)))
        print("[MeleeRNG] firetouchinterest:", tostring(type(_firetouchinterest)))
        SLbl.Text = "Touch APIs: " .. touchApiStatus()
    end)
...
meleeGuiMountHitboxPulseSection(MoveTab, SLbl, C)
...
local _perfWeaponSaved = { enabled = {}, folderConns = {}, modelConns = {}, stash = nil } -- deeper weapon hiding: stash visual parts outside workspace + restore
...
local function perfCleanupStalePartKeys()
    for p, saved in pairs(_perfPartLTM) do
        if not p then
            _perfPartLTM[p] = nil
        elseif type(saved) == "table" then
            -- Stashed visual parts live under PlayerGui while hidden, so keep them in
            -- the restore map. Drop only if the saved original parent vanished.
            if saved.parent == nil and not p.Parent then
                _perfPartLTM[p] = nil
            end
...
-- Attack-safe weapon hiding:
-- Hide only visuals. Do NOT touch BaseParts named Hitbox, because the local
-- attack / pulse path depends on those parts staying normal, touchable, and cached.
-- v5: LocalTransparencyModifier alone still leaves every visual part in workspace,
-- so high weapon counts can keep rendering/physics/particle work alive. Hide Own
-- Weapons now stashes non-Hitbox visual parts under PlayerGui, disables weapon VFX,
-- and restores everything when turned off.
local function perfIsCombatHitboxPart(inst)
    return inst and inst:IsA("BasePart") and inst.Name == "Hitbox"
end

local function perfGetWeaponStash()
    if _perfWeaponSaved.stash and _perfWeaponSaved.stash.Parent then
        return _perfWeaponSaved.stash
    end
    local pg = LP and LP:FindFirstChildOfClass("PlayerGui")
    if not pg then return nil end
    local stash = pg:FindFirstChild("MeleeRNG_HiddenWeaponVisuals")
    if not stash then
        stash = Instance.new("Folder")
        stash.Name = "MeleeRNG_HiddenWeaponVisuals"
        stash.Parent = pg
    end
    _perfWeaponSaved.stash = stash
    return stash
end

local function perfIsWeaponEffectLike(inst)
    return inst:IsA("ParticleEmitter") or inst:IsA("Trail") or inst:IsA("Beam")
        ...
        or inst:IsA("BillboardGui") or inst:IsA("SurfaceGui") or inst:IsA("Highlight")
end

local function perfSetWeaponEffectHidden(inst, hide)
    if not (inst and inst.Parent and perfIsWeaponEffectLike(inst)) then return end
    local saved = _perfWeaponSaved.enabled
    if hide then
        if saved[inst] == nil then
            local ok, enabled = pcall(function() return inst.Enabled end)
            if ok then saved[inst] = enabled end
        end
        pcall(function() inst.Enabled = false end)
    else
        local prev = saved[inst]
        if prev ~= nil then
            pcall(function()
                if inst.Parent then inst.Enabled = prev end
            end)
            saved[inst] = nil
        end
    end
...
local function perfRestoreVisualPart(part)
    local prev = _perfPartLTM[part]
    if prev == nil then return end

    if type(prev) == "table" then
        pcall(function()
            if prev.parent and part.Parent ~= prev.parent then
                part.Parent = prev.parent
            end
            if part.Parent then
                part.LocalTransparencyModifier = prev.ltm or 0
                if prev.canTouch ~= nil then part.CanTouch = prev.canTouch end
                if prev.canQuery ~= nil then part.CanQuery = prev.canQuery end
                if prev.canCollide ~= nil then part.CanCollide = prev.canCollide end
                if prev.castShadow ~= nil then part.CastShadow = prev.castShadow end
            end
        end)
    else
        pcall(function()
            if part.Parent then part.LocalTransparencyModifier = prev end
        end)
    end
    _perfPartLTM[part] = nil
end

local function perfSetVisualPartHidden(part, hide, ownerFolder)
    if not (part and part:IsA("BasePart")) then return end
    if perfIsCombatHitboxPart(part) then
        -- Keep combat hitboxes out of Hide Own Weapons.
        -- The separate "Hide Hitbox visuals" toggle can hide the ring if needed.
        return
    end

    if hide then
        if _perfPartLTM[part] == nil then
            _perfPartLTM[part] = {
        ...
        local stash = perfGetWeaponStash()
        if stash and part.Parent ~= stash then
            pcall(function() part.Parent = stash end)
        end
    else
        perfRestoreVisualPart(part)
    end
end

local function perfDisconnectWeaponModel(model)
    local c = _perfWeaponSaved.modelConns[model]
    if c then
        pcall(function() c:Disconnect() end)
        _perfWeaponSaved.modelConns[model] = nil
    end
end

local function perfApplyWeaponDescendant(d, hide, ownerFolder)
    if d:IsA("BasePart") then
        perfSetVisualPartHidden(d, hide, ownerFolder)
    elseif perfIsWeaponEffectLike(d) then
        perfSetWeaponEffectHidden(d, hide)
    end
end

local function perfTrackWeaponModel(model, hide, ownerFolder)
    if not (model and model:IsA("Model")) then return end
    if hide then
        for _, d in ipairs(model:GetDescendants()) do
            perfApplyWeaponDescendant(d, true, ownerFolder)
        end
        if not _perfWeaponSaved.modelConns[model] then
            _perfWeaponSaved.modelConns[model] = model.DescendantAdded:Connect(function(d)
                if (states.perfHideOwnWeapons == true and ownerFolder == workspace:FindFirstChild(LP.Name)) or states.perfHideOtherWeapons == true then
                    task.defer(function()
                        if d and d.Parent then perfApplyWeaponDescendant(d, true, ownerFolder) end
                    end)
                end
            end)
            addConn(_perfWeaponSaved.modelConns[model])
        end
    else
        perfDisconnectWeaponModel(model)
        for _, d in ipairs(model:GetDescendants()) do
            perfApplyWeaponDescendant(d, false, ownerFolder)
        end
    end
end

local function perfRestoreWeaponFolderSaved(folder)
    for part, saved in pairs(_perfPartLTM) do
        if part and type(saved) == "table" and saved.owner == folder then
            perfRestoreVisualPart(part)
        end
    end
    for inst in pairs(_perfWeaponSaved.enabled) do
        pcall(function()
            if inst and inst.Parent and folder and inst:IsDescendantOf(folder) then
                perfSetWeaponEffectHidden(inst, false)
            end
        end)
    end
end
...
local function perfSweepWeaponFolder(folder, hide)
    ...
    if hide then
        for _, m in ipairs(folder:GetChildren()) do
            if m:IsA("Model") then
                perfTrackWeaponModel(m, true, folder)
            end
        end
        if not _perfWeaponSaved.folderConns[folder] then
            _perfWeaponSaved.folderConns[folder] = folder.ChildAdded:Connect(function(ch)
                if ch:IsA("Model") then
                    task.defer(function()
                        if ch.Parent and ((folder == workspace:FindFirstChild(LP.Name) and states.perfHideOwnWeapons == true) or states.perfHideOtherWeapons == true) then
                            perfTrackWeaponModel(ch, true, folder)
                        end
                    end)
                end
            end)
            addConn(_perfWeaponSaved.folderConns[folder])
        end
    else
        local c = _perfWeaponSaved.folderConns[folder]
        if c then
            pcall(function() c:Disconnect() end)
            _perfWeaponSaved.folderConns[folder] = nil
        end
        for _, m in ipairs(folder:GetChildren()) do
            if m:IsA("Model") then
                perfTrackWeaponModel(m, false, folder)
            end
        end
        perfRestoreWeaponFolderSaved(folder)
    end
end
...
-- ── Mob performance + local spawn-on-player controller ─────────────────────
-- Event-driven so mob count does not force a workspace:GetDescendants() sweep every frame.
-- Keeps each mob's HumanoidRootPart / PrimaryPart in workspace for targeting, but stashes
-- non-root visual parts and disables mob VFX when hard-hide is ON.
local function meleeInitMobSpawnPerfSystem()
    local api = {}
    local folderConn = nil
    local folderWaitThread = nil
    local tracked = {}          -- [Model] = true
    local rootList = {}         -- compact-ish list of tracked mob models for batch magnet
    local rootIndex = 1
    local savedParts = {}       -- [BasePart] = {parent, ltm, canTouch, canQuery, canCollide, castShadow, owner}
    local savedFx = {}          -- [Instance] = {enabled, owner}
    local descConns = {}        -- [Model] = RBXScriptConnection
    local stash = nil

    local MOB_TP_BATCH = 40
    local MOB_TP_TICK_SEC = 0.08
    local MOB_TRACK_RESCAN_SEC = 2.5
    ...
    local function getStash()
        if stash and stash.Parent then return stash end
        local pg = LP and LP:FindFirstChildOfClass("PlayerGui")
        if not pg then return nil end
        stash = pg:FindFirstChild("MeleeRNG_HiddenMobVisuals")
        if not stash then
            stash = Instance.new("Folder")
            stash.Name = "MeleeRNG_HiddenMobVisuals"
            stash.Parent = pg
        end
        return stash
    end

    local function isFx(inst)
        return inst:IsA("ParticleEmitter") or inst:IsA("Trail") or inst:IsA("Beam")
            or inst:IsA("Fire") or inst:IsA("Smoke") or inst:IsA("Sparkles")
            or inst:IsA("PointLight") or inst:IsA("SpotLight") or inst:IsA("SurfaceLight")
            or inst:IsA("BillboardGui") or inst:IsA("SurfaceGui") or inst:IsA("Highlight")
    ...
    local function isMobModel(m)
        ...
        if folder and m.Parent ~= folder then return false end
        return m:GetAttribute("ID") ~= nil or findRoot(m) ~= nil
    end

    local function setFxHidden(inst, owner, hide)
        if not (inst and inst.Parent and isFx(inst)) then return end
        if hide then
            if savedFx[inst] == nil then
                local ok, enabled = pcall(function() return inst.Enabled end)
                if ok then savedFx[inst] = { enabled = enabled, owner = owner } end
            end
            pcall(function() inst.Enabled = false end)
        else
            local rec = savedFx[inst]
            if rec ~= nil then
                pcall(function()
                    if inst.Parent then inst.Enabled = rec.enabled end
                end)
                savedFx[inst] = nil
            end
        end
    ...
    local function restorePart(part)
        local rec = savedParts[part]
        if rec == nil then return end
        pcall(function()
            if rec.parent and part.Parent ~= rec.parent then
                part.Parent = rec.parent
            end
            if part.Parent then
                part.LocalTransparencyModifier = rec.ltm or 0
                if rec.canTouch ~= nil then part.CanTouch = rec.canTouch end
                if rec.canQuery ~= nil then part.CanQuery = rec.canQuery end
                if rec.canCollide ~= nil then part.CanCollide = rec.canCollide end
                if rec.castShadow ~= nil then part.CastShadow = rec.castShadow end
            end
        end)
        savedParts[part] = nil
    end

    local function hidePart(part, owner, root)
        if not (part and part:IsA("BasePart")) then return end
        local isRoot = (part == root) or part.Name == "HumanoidRootPart"
        if savedParts[part] == nil then
            savedParts[part] = {
                parent = part.Parent,
                ltm = part.LocalTransparencyModifier,
                canTouch = part.CanTouch,
        ...
        pcall(function()
            ...
            if not isRoot then
                ...
                -- Keep root touch/query active so hitbox/nearest-mob logic still works.
                part.CanTouch = true
                part.CanQuery = true
            end
        end)
        if not isRoot then
            local st = getStash()
            if st and part.Parent ~= st then
                pcall(function() part.Parent = st end)
            end
        end
    end

    local function cleanupDeadKeys()
        for part, rec in pairs(savedParts) do
            if not part or (not part.Parent and (not rec or not rec.parent)) then
                savedParts[part] = nil
            end
        end
        for inst in pairs(savedFx) do
            if not (inst and inst.Parent) then savedFx[inst] = nil end
        end
        local i = 1
        while i <= #rootList do
            local m = rootList[i]
            if not (m and m.Parent and tracked[m]) then
                table.remove(rootList, i)
            else
                i = i + 1
            end
        end
        if rootIndex > #rootList then rootIndex = 1 end
    end

    local function restoreMob(m)
        local dc = descConns[m]
        if dc then
            pcall(function() dc:Disconnect() end)
            descConns[m] = nil
        end
        for part, rec in pairs(savedParts) do
            if rec and rec.owner == m then restorePart(part) end
        end
        for inst, rec in pairs(savedFx) do
            if rec and rec.owner == m then setFxHidden(inst, m, false) end
        end
    end

    local function applyDescendant(m, d, hide, root)
        if d:IsA("BasePart") then
            if hide then hidePart(d, m, root) else restorePart(d) end
        elseif isFx(d) then
            setFxHidden(d, m, hide)
        end
    end

    local function hideMob(m, hide)
        if not isMobModel(m) then return end
        local root = findRoot(m)
        if hide then
            for _, d in ipairs(m:GetDescendants()) do
                applyDescendant(m, d, true, root)
            end
            if not descConns[m] then
                descConns[m] = m.DescendantAdded:Connect(function(d)
                    if states.perfHardHideMobs == true then
                        task.defer(function()
                            if m.Parent and d.Parent then applyDescendant(m, d, true, findRoot(m)) end
                        end)
                    end
                end)
                addConn(descConns[m])
    ...
    local function teleportMob(m, playerRoot)
        if not (states.mobSpawnOnPlayer == true and isMobModel(m)) then return false end
        local root = findRoot(m)
        playerRoot = playerRoot or getRoot()
        if not (root and playerRoot) then return false end
        local target = playerRoot.CFrame * CFrame.new(0, 0, -2)
        pcall(function()
            root.AssemblyLinearVelocity = Vector3.zero
            root.AssemblyAngularVelocity = Vector3.zero
        end)
    ...
    local function trackMob(m, instant)
        if not isMobModel(m) or tracked[m] then return end
        tracked[m] = true
        rootList[#rootList + 1] = m
        if states.perfHardHideMobs == true then hideMob(m, true) end
        if states.mobSpawnOnPlayer == true then
            task.spawn(function()
                for _ = 1, 20 do
                    if GEN ~= _G.MeleeRNG_Gen or not m.Parent then return end
                    if teleportMob(m) then break end
                    task.wait(instant and 0.02 or 0.05)
                end
            end)
        end
    end
    ...
    local function scanFolder()
        ...
        if not folder then return end
        for _, m in ipairs(folder:GetChildren()) do
            if m:IsA("Model") then trackMob(m, false) end
        end
        cleanupDeadKeys()
    end

    local function ensureFolderConn()
        local folder = getFolder()
        if folder then
            if not folderConn then
                folderConn = folder.ChildAdded:Connect(function(ch)
                    if ch:IsA("Model") then
                        task.defer(function() trackMob(ch, true) end)
                    end
                end)
                addConn(folderConn)
            end
        ...
        if not folderWaitThread then
            folderWaitThread = task.spawn(function()
                while GEN == _G.MeleeRNG_Gen and (states.mobSpawnOnPlayer == true or states.perfHardHideMobs == true) do
                    if ensureFolderConn() then break end
                    task.wait(0.5)
                end
                folderWaitThread = nil
            end)
            addThread(folderWaitThread)
        end
        return false
    end

    function api.setSpawnOnPlayer(on, SLbl)
        states.mobSpawnOnPlayer = on == true
        if states.mobSpawnOnPlayer then
            ensureFolderConn()
            scanFolder()
            if SLbl then SLbl.Text = "Mobs: new spawns move onto you" end
        elseif SLbl then
            SLbl.Text = "Mobs: spawn-on-player OFF"
        end
    end

    function api.setHardHide(on, SLbl)
        states.perfHardHideMobs = on == true
        ensureFolderConn()
        if states.perfHardHideMobs then
            scanFolder()
            for _, m in ipairs(rootList) do
                if m and m.Parent then hideMob(m, true) end
            end
            if SLbl then SLbl.Text = "Perf: mob visuals stashed; roots kept active" end
        else
            for m in pairs(tracked) do restoreMob(m) end
            cleanupDeadKeys()
            if SLbl then SLbl.Text = "Perf: mob visuals restored" end
        end
    end

    function api.tpAllNow(SLbl)
        ensureFolderConn()
        scanFolder()
        local root = getRoot()
        local n = 0
        if root then
            for _, m in ipairs(rootList) do
                if m and m.Parent and teleportMob(m, root) then n = n + 1 end
            end
        end
        if SLbl then SLbl.Text = "Mobs moved to you: " .. tostring(n) end
    end

    function api.restoreAll()
        states.mobSpawnOnPlayer = false
        states.perfHardHideMobs = false
        if folderConn then pcall(function() folderConn:Disconnect() end); folderConn = nil end
        for m in pairs(tracked) do restoreMob(m) end
        tracked = {}
        rootList = {}
        cleanupDeadKeys()
    ...
    function api.applySaved(SLbl)
        if states.mobSpawnOnPlayer == true or states.perfHardHideMobs == true then
            ensureFolderConn()
            scanFolder()
            if states.perfHardHideMobs == true then api.setHardHide(true, nil) end
            if SLbl then SLbl.Text = "Mob performance settings applied" end
        end
    end
        ...
        while true do
            ...
            if states.mobSpawnOnPlayer == true then
                ...
                if now >= rescanAt then
                    rescanAt = now + MOB_TRACK_RESCAN_SEC
                    scanFolder()
                end
                local playerRoot = getRoot()
                if playerRoot and #rootList > 0 then
                    local limit = math.min(MOB_TP_BATCH, #rootList)
                    for _ = 1, limit do
                        local m = rootList[rootIndex]
                        rootIndex = rootIndex + 1
                        if rootIndex > #rootList then rootIndex = 1 end
                        if m and m.Parent then teleportMob(m, playerRoot) end
                    end
                end
                task.wait(MOB_TP_TICK_SEC)
            else
                task.wait(0.35)
            end
        end
...
local mobPerfApi = meleeInitMobSpawnPerfSystem()
...
addThread(task.spawn(function()
    while true do
        ...
        -- Weapon hiding is event-assisted and stashes visual parts outside workspace,
        -- so we only need a slow fallback sweep for newly-created player folders.
        task.wait((ow or sw or (hv and not sw)) and 0.75 or 0.18)
        ...
        local doWeaponSweep = (_perfWeaponAcc == 1 or _perfWeaponAcc >= 3)
        if _perfWeaponAcc >= 3 then _perfWeaponAcc = 1 end

        if sw then
            if doWeaponSweep then perfSweepWeaponFolder(workspace:FindFirstChild(LP.Name), true) end
        ...
        if doWeaponSweep and (sw or ow or _perfLastOwnWeaponsHidden or _perfLastOtherWeaponsHidden) then
            perfCleanupStalePartKeys()
        end
...
local autoEquipBestToggle = MoveTab:Toggle("⚔️ Auto Equip Best", "Fires EquipBest on a slower adjustable interval to avoid weapon-rebuild lag spikes.", states.autoEquipBest, function(on)
...
MoveTab:Slider("Auto Equip Best interval", "Higher = fewer lag spikes. Default 45s.", {min=15, max=300, default=states.autoEquipBestInterval}, function(v)
    states.autoEquipBestInterval = math.clamp(math.floor(tonumber(v) or 45), 15, 300)
    saveSettings()
    SLbl.Text = "Auto Equip Best interval: " .. tostring(states.autoEquipBestInterval) .. "s"
end)
task.defer(function()
    pcall(function()
...
-- v6: slower/adaptive loop. EquipBest can rebuild equipped weapon models; firing it
-- every 10s causes a visible hitch when many swords exist. Keep it periodic, but much
-- less spammy, and immediately re-apply visual stashing after the weapon folder rebuilds.
addThread(task.spawn(function()
    ...
    while true do
        ...
        local interval = math.clamp(math.floor(tonumber(states.autoEquipBestInterval) or 45), 15, 300)
        local now = os.clock()
        if now - lastEquipBest < interval then continue end
        lastEquipBest = now
        pcall(function()
            fireR("EquipBest")
            _hitboxCacheDirty = true
        end)
        if states.perfHideOwnWeapons == true then
            task.delay(0.25, function()
                if GEN ~= _G.MeleeRNG_Gen or _stopped then return end
                pcall(function() perfSweepWeaponFolder(workspace:FindFirstChild(LP.Name), true) end)
            end)
        end
    end
end))
...
;(function()
    ...
    meleeGuiMountDungeonTpSection(MiscTab, SLbl, C)
    meleeGuiMountTimeTrialSection(MiscTab, SLbl, C)
    meleeGuiMountClientRoomSection(MiscTab, SLbl, C)

    MiscTab:Section("Performance")
    MiscTab:Label("Client-only FPS helpers. Hide Own Weapons now skips combat Hitbox parts and stashes visual parts outside workspace to cut render/physics lag.")
    local perfUi = {}
    perfUi.other = MiscTab:Toggle("Hide Other Players' Weapons", "Hides visual parts under workspace[their name], skips parts named Hitbox, throttled for FPS — saved", states.perfHideOtherWeapons, function(on)
        states.perfHideOtherWeapons = on == true
        saveSettings()
        SLbl.Text = on and "Perf: others’ weapons hidden" or "Perf: others’ weapons shown"
    end)
    perfUi.own = MiscTab:Toggle("Hide Own Weapons", "Attack-safe: stashes visual weapon parts under PlayerGui, skips parts named Hitbox, and restores when off — saved", states.perfHideOwnWeapons, function(on)
        states.perfHideOwnWeapons = on == true
        saveSettings()
        pcall(function() perfSweepWeaponFolder(workspace:FindFirstChild(LP.Name), on == true) end)
        _perfWeaponAcc = 0
        SLbl.Text = on and "Perf: your weapon visuals stashed (Hitboxes kept active)" or "Perf: your weapons restored"
    end)
    ...
    perfUi.mobhard = MiscTab:Toggle("Hard-hide mob models", "Stronger than the game HideMobs button: stashes non-root mob visual parts under PlayerGui, disables mob VFX, keeps HumanoidRootPart active for targeting — saved", states.perfHardHideMobs, function(on)
        mobPerfApi.setHardHide(on == true, SLbl)
        saveSettings()
    end)
    perfUi.mobtp = MiscTab:Toggle("Mobs spawn on me", "Watches workspace.Mobs.ChildAdded and moves each mob root to your character. Also batch-catches existing mobs without heavy full sweeps — saved", states.mobSpawnOnPlayer, function(on)
        mobPerfApi.setSpawnOnPlayer(on == true, SLbl)
        saveSettings()
    end)
    MiscTab:Button("📍", "Move all mobs to me now", "One-shot batch move of currently tracked workspace.Mobs models to your HumanoidRootPart.", C.accent, function()
        mobPerfApi.tpAllNow(SLbl)
    end)
    ...
    task.defer(function()
        pcall(function()
            ...
            if p.mobhard and p.mobhard.Set then p.mobhard.Set(states.perfHardHideMobs) end
            if p.mobtp and p.mobtp.Set then p.mobtp.Set(states.mobSpawnOnPlayer) end
            ...
            if states.perfHardHideMobs or states.mobSpawnOnPlayer then pcall(function() mobPerfApi.applySaved(SLbl) end) end
...
MiscTab:Button("🛑", "Stop All Loops", "Disconnects all connections and cancels all threads", C.red, function()
    ...
    states.mobSpawnOnPlayer = false
    states.perfHardHideMobs = false
    pcall(function() mobPerfApi.restoreAll() end)
