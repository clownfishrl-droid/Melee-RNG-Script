...
local states = {
    ...
    dungeonServerRegisteredOnly = true, -- skip local client-created Room_N models; use real server-streamed rooms only
...
-- ON by default: local Room_N models are useful for testing, but they cannot
-- make server-side stage/progress counters move. Keep real/server rooms only
-- for progression-related teleports.
states.dungeonServerRegisteredOnly = states.dungeonServerRegisteredOnly ~= false
...
local function dungeonIsClientMadeRoom(room)
    return room and room:GetAttribute("MeleeRNG_ClientRoomNumber") == true
end

local function dungeonRoomAllowedForProgress(room)
    if not (room and room:IsA("Model")) then return false end
    -- Server registration can only happen on real rooms streamed/created by the server.
    -- Local test rooms remain useful, but are skipped while this safety is ON.
    if states.dungeonServerRegisteredOnly and dungeonIsClientMadeRoom(room) then
        return false
    end
    return true
end
...
local function dungeonFindLatestRoom()
    ...
    for _, child in ipairs(folder:GetChildren()) do
        if dungeonRoomAllowedForProgress(child) then
...
local function dungeonFindRoomByNumber(value)
    ...
    if dungeonRoomAllowedForProgress(room) then return room, n end
    return nil, n
end
...
local function dungeonScheduleTp(room, immediate, SLbl)
    ...
    if not dungeonRoomAllowedForProgress(room) then return end
...
local function dungeonScheduleTargetTp(room, SLbl)
    if not dungeonRoomAllowedForProgress(room) then
        if SLbl then SLbl.Text = "Dungeon target TP: waiting for real server Room_N" end
        return
    end
...
local function dungeonSetAutoTpTargetRoom(on, SLbl)
    ...
    _dungeonTargetTpConn = folder.ChildAdded:Connect(function(child)
        if states.dungeonAutoTpTargetRoom ~= true or not dungeonRoomAllowedForProgress(child) then return end
        local idx = dungeonRoomIndex(child)
...
local function dungeonSetAutoTp(on, SLbl)
    ...
    _dungeonTpConn = folder.ChildAdded:Connect(function(child)
        if states.dungeonAutoTp == true and dungeonRoomAllowedForProgress(child) then
...
local function meleeGuiMountDungeonTpSection(MiscTab, SLbl, C)
    ...
    MiscTab:Label("Server progress only counts real server-created Room_N models. Client-created Room_N test rooms are skipped while Server rooms only is ON.")
    MiscTab:Toggle("✅ Server rooms only", "ON = ignore locally-created Room_N models so TP targets can be registered by the server normally. Saved.", states.dungeonServerRegisteredOnly, function(on)
        states.dungeonServerRegisteredOnly = on ~= false
        saveSettings()
        if states.dungeonAutoTp == true then dungeonSetAutoTp(true, SLbl) end
        if states.dungeonAutoTpTargetRoom == true then dungeonSetAutoTpTargetRoom(true, SLbl) end
        SLbl.Text = states.dungeonServerRegisteredOnly and "Dungeon: real server rooms only" or "Dungeon: local test rooms allowed"
    end)
    ...
    MiscTab:Toggle("🏰 Auto TP new rooms", "Watches GeneratedDungeon.ChildAdded; teleports to new real Room_N → BASE. Saved.", states.dungeonAutoTp, function(on)
    ...
    MiscTab:Button("➕", "Create local test Room_N BASE", "Client-only testing room. This will NOT update server stage/progress; use real server rooms for that.", C.green, function()
        dungeonCreateClientRoomNumberAtPlayer(SLbl)
    end)
    MiscTab:Button("🎯", "TP entered REAL Room_N now", "Finds the server-created room number you entered and teleports above its BASE once.", C.purple, function()
        local room, n = dungeonFindRoomByNumber(states.dungeonTargetRoomNumber)
        local ok, why = dungeonTpToRoom(room)
        SLbl.Text = ok and ("Dungeon TP: Room_" .. tostring(n) .. " / " .. tostring(why)) or ("Dungeon TP failed for real Room_" .. tostring(n or "?") .. ": " .. tostring(why))
    end)
...
-- Tabs: Move → Dungeon → ESP → Areas → Upgrades → Sacrifice → Guild → Ascend AP → (later) Misc
local MoveTab = Win:Tab("🚀",  "Move")
_G.MeleeRNG_DungeonTab = Win:Tab("🏯", "Dungeon")
...
-- Mount Dungeon tab after the main UI IIFE returns. This keeps the heavy dungeon
-- section builders out of the already-large UI scope, avoiding Luau local/register
-- limit issues that can prevent the whole window from showing.
task.defer(function()
    local tab = rawget(_G, "MeleeRNG_DungeonTab")
    local c = rawget(_G, "MeleeRNG_UIC")
    if not tab or not c then return end
    local ok, err = pcall(function()
        meleeGuiMountDungeonTpSection(tab, SLbl, c)
        meleeGuiMountClientRoomSection(tab, SLbl, c)
    end)
    if not ok then
        warn("[MeleeRNG] Dungeon tab mount failed: " .. tostring(err))
        if SLbl then SLbl.Text = "Dungeon tab failed: " .. tostring(err) end
    end
end)
