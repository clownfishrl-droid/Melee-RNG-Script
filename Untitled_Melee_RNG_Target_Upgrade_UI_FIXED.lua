...
-- --------------------------------------------------------------------------
local Win  = Lib:Window({ title = "⚔️  Melee RNG  v1.1.1 Target Upgrades" })
...
for _, name in ipairs(UPGRADE_ORDER) do
    ...
    local n2    = name
    upgToggles[name] = UpgTab:Toggle(name, ((UPGRADE_DESCRIPTIONS[n2] or "SP upgrade.") .. " Target Chain: ON + target level = buy this before moving to the next target."), isSel, function(on)
