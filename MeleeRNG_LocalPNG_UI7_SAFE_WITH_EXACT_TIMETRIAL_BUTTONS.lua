...
local function meleeRobustClickGuiTarget(obj, reason)
    ...
    elseif obj:IsA("TextButton") or obj:IsA("ImageButton") then
        signaled = meleeFireButtonSignals(obj)
    end
    ...
    local clickedCenter = meleeVirtualClickGuiObject(obj, 3)
    if not clickedCenter and btn and btn ~= obj then
        clickedCenter = meleeVirtualClickGuiObject(btn, 3)
    end
    if not clickedCenter and obj.Parent and obj.Parent:IsA("GuiObject") and meleeGuiIsActuallyVisible(obj.Parent) then
        -- Some TimeTrial buttons use the parent frame as the actual click target.
        clickedCenter = meleeVirtualClickGuiObject(obj.Parent, 2)
    end
