...
local states = {
    ...
    timeTrialLeaveAtRoom = 75, -- Time Trial room threshold for auto leave/restart
...
states.timeTrialLeaveAtRoom = math.clamp(math.floor(tonumber(states.timeTrialLeaveAtRoom) or 75), 1, 1000000)
...
function meleeMountSafeDungeonMobTimeTrialAddons(MiscTab, SLbl, C)
    ...
    MiscTab:Input("Leave/restart at Room_N", "Default is 75. When Room_N exists, click LeaveTimeTrialBtn, then StartTimeTrial again.", "75", tostring(states.timeTrialLeaveAtRoom), function(txt)
        states.timeTrialLeaveAtRoom = math.clamp(math.floor(tonumber(tostring(txt):match("%d+") or txt) or 75), 1, 1000000)
        saveSettings()
    ...
    addThread(task.spawn(function()
        ...
        while true do
            ...
            if states.timeTrialAutoLeaveRestart then
                local target = math.clamp(math.floor(tonumber(states.timeTrialLeaveAtRoom) or 75), 1, 1000000)
