...
local function uiBgStartFrameAnimation(img, assets)
    ...
    elseif buffer and buffer:IsA("ImageLabel") then
        buffer.Size = img.Size
        buffer.Position = img.Position
        buffer.BackgroundTransparency = 1
        buffer.ZIndex = 2
    ...
    task.spawn(function()
        local i = 1
        while GEN == _G.MeleeRNG_Gen and token == _uiBgAnimToken and img and img.Parent and #assets < 30 do
            img.Image = assets[1]
            task.wait(0.05)
        end
            ...
            if count >= 2 then
                ...
                if type(nextImage) == "string" and nextImage ~= "" then
                    if buffer and buffer.Parent then
                        ...
                        local deadline = os.clock() + math.max(delay * 3, 0.12)
                        local loaded = false
