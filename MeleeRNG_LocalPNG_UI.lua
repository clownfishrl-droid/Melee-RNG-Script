...
local UI_BG_PROBE_NAME = "MeleeRNG_LocalPng_Probe"
...
local function uiBgEnsureProbe()
    local pg = LP and LP:FindFirstChildOfClass("PlayerGui")
    if not pg then return nil end
    local probe = pg:FindFirstChild(UI_BG_PROBE_NAME)
    if not probe then
        probe = Instance.new("ImageLabel")
        probe.Name = UI_BG_PROBE_NAME
        probe.Size = UDim2.new(0, 1, 0, 1)
        probe.Position = UDim2.new(0, -10000, 0, -10000)
...
local function uiBgImageLoads(asset, timeoutSec)
    if type(asset) ~= "string" or asset == "" then return false end
    local probe = uiBgEnsureProbe()
    if not probe then return true end
    probe.Image = ""
    probe.Image = asset
    uiBgPreloadAssets({ asset })
    local deadline = os.clock() + (timeoutSec or 0.55)
    while os.clock() < deadline do
        local ok, loaded = pcall(function() return probe.IsLoaded end)
        if ok and loaded == true then return true end
        task.wait(0.03)
    end
    return false
end
...
local function uiBgEnsureLocalPngFrames()
    ...
    for _, folder in ipairs(UI_BG_LOCAL_FRAME_FOLDERS) do
        local firstPath = uiBgLocalFramePath(folder, 1)
        if uiBgFileExists(firstPath) then
            local firstAsset = uiBgGetCustomAsset(firstPath)
            if firstAsset then
                if not uiBgImageLoads(firstAsset, 1.2) then
                    lastProblem = "first PNG asset did not decode: " .. firstPath
                    vprint("[MeleeRNG PNG BG] " .. lastProblem)
                else
                    local assets = { firstAsset }
                    _uiBgCachedLocalAssets = assets
                    UI_BG_LAST_STATUS = string.format("showing decoded PNG frame 1 from %s; checking rest", folder)
                    ...
                    task.spawn(function()
                        for i = 2, UI_BG_LOCAL_FRAME_COUNT do
                            ...
                            local path = uiBgLocalFramePath(folder, i)
                            ...
                            if not asset or not uiBgImageLoads(asset, 0.45) then
                                UI_BG_LAST_STATUS = "skipped PNG frame that did not decode: " .. path
                                vprint("[MeleeRNG PNG BG] " .. UI_BG_LAST_STATUS)
                            else
...
local function uiBgIsOwnObject(obj)
    return obj and (
        ...
        obj.Name == UI_BG_STATUS_NAME or
        obj.Name == UI_BG_PROBE_NAME
    )
end
...
local function uiBgStartFrameAnimation(img, assets)
    ...
    if not buffer and img.Parent then
        ...
        buffer.ZIndex = img.ZIndex
        buffer.Visible = false
        buffer.ImageTransparency = 0
        buffer.Parent = img.Parent
    ...
    task.spawn(function()
        ...
        while GEN == _G.MeleeRNG_Gen and token == _uiBgAnimToken and img and img.Parent do
            ...
            if count >= 2 then
                ...
                local nextImage = assets[i]
                if type(nextImage) == "string" and nextImage ~= "" then
                    if buffer and buffer.Parent then
                        buffer.Visible = false
                        buffer.Image = nextImage
                        local deadline = os.clock() + math.max(delay * 2, 0.08)
                        local loaded = false
                        while os.clock() < deadline do
                            local ok, isLoaded = pcall(function() return buffer.IsLoaded end)
...
local function meleeApplyLocalPngBackground(enabled)
    ...
    local oldBuffer = layer:FindFirstChild(UI_BG_BUFFER_NAME)
    if oldBuffer and oldBuffer:IsA("ImageLabel") then
        oldBuffer.BackgroundTransparency = 1
        oldBuffer.Visible = false
        uiBgTrySet(oldBuffer, "ScaleType", Enum.ScaleType.Crop)
    end
    ...
    if assets and #assets > 0 then
        ...
        UI_BG_LAST_STATUS = string.format("using visible local PNG frames (%d/%d at %d FPS, looping)", #assets, UI_BG_LOCAL_FRAME_COUNT, UI_BG_LOCAL_FRAME_FPS)
        uiBgSetInlineStatus(root, "", false)
    else
...
MiscTab:Toggle("🎥 Local PNG Background", "ON = animate MeleeRNG_UI_Background_manifest/extracted_frames behind this UI for only your client.", states.uiPngBackground, function(on)
