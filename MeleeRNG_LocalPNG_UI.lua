    ...
    for _, folder in ipairs(UI_BG_LOCAL_FRAME_FOLDERS) do
        ...
        if uiBgFileExists(firstPath) then
            ...
            if firstAsset then
                ...
                UI_BG_LAST_STATUS = string.format("showing local PNG frame 1 from %s; loading rest", folder)
                task.spawn(function()
                    ...
                    UI_BG_LAST_STATUS = string.format("using local PNG frames from %s (%d/%d at %d FPS)", folder, #assets, UI_BG_LOCAL_FRAME_COUNT, UI_BG_LOCAL_FRAME_FPS)
                end)
                return assets
            end
